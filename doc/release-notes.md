...
- status: available
- changes:
  - Ependymoma subtyping module was corrected to capture `YAP1--MAML2` per [ticket 1618](https://github.com/AlexsLemonade/OpenPBTA-analysis/issues/1618) and `pbta-histologies.tsv` has been updated to include newly subtyped samples.
  - Rerun of `fusion_filtering` resulted in a minor update to `pbta-fusion-recurrent-fusion-bysample.tsv` and `pbta-fusion-recurrently-fused-genes-byhistology.tsv` to correctly capture _EWSR1::FLI1_ recurrent fusions/genes (see PR review [here](https://github.com/AlexsLemonade/OpenPBTA-analysis/pull/1631#pullrequestreview-1219972909).
```
data
...
- status: available
- changes:
  - Update broad_histology and short_histology in pbta-histologies-base.tsv as per  [#1155](https://github.com/AlexsLemonade/OpenPBTA-analysis/issues/1155)
  - Added pathology_diagnosis == Subependymal Giant Cell Astrocytoma (SEGA) to LGAT subtyping [#1154](https://github.com/AlexsLemonade/OpenPBTA-analysis/issues/1154)
  - cancer_group and harmonized_diagnosis updated in pbta-histologies.tsv as per [#1149](https://github.com/AlexsLemonade/OpenPBTA-analysis/issues/1149) 
        - Ganglioglioma and Glial-neuronal tumors were added to LGAT subtyping but integrated_diagnosis was not accurately updated in previous release
        - Additionally latest mapping file for pathology_diagnosis == "Other" was used to correspond to terms in files used in [#1155](https://github.com/AlexsLemonade/OpenPBTA-analysis/issues/1155)
- folder structure
```
data
└── release-v21-20210820
