...
```{r}
module_contributions_file <- file.path("..", 
                                       "analyses", 
                                       "count-contributions", 
                                       "results", 
...
pinned_authors <- c("Joshua A. Shapiro", 
                    "Adam C. Resnick", 
                    "Casey S. Greene",
                    # Asterisks are in the name field for corresponding authors
                    "Jo Lynne Rokita*",  
                    "Jaclyn N. Taroni*",
...
code_contributors <- module_contributions_df %>%
  # For corresponding authors, we need to add an * to their names in order to
  # filter them out in the next step
  mutate(author = if_else(author %in% c("Jaclyn N. Taroni", "Jo Lynne Rokita"),
                          str_c(author, "*"),
                          author)
  ) %>%
...
yaml::write_yaml(updated_manuscript_metadata, "updated_metadata.yaml")
```
