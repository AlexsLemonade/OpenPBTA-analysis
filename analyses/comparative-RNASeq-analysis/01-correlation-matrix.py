...
def main():
    ...
    p.add_argument("--scratch", help="Path to scratch dir.", default=os.path.join("..", "..", "scratch"))
    p.add_argument("--output-prefix", help="Prefix for output files.")
    p.add_argument("--proportion-unexpressed", default=0.8, type=float, help="Threshold for dropping genes expressed in too few samples.")
    p.add_argument("--variance-filter-level", default=0.2, type=float, help="Threshold for what proportion of the least varying genes to drop.")
    args = p.parse_args()
    ...
    expression = prepare_expression(args.input_path, args.scratch, prefix=prefix, verbose=args.verbose,
                                     proportion_unexpressed=args.proportion_unexpressed, variance_filter_level=args.variance_filter_level)
    calculate_correlation(expression, args.scratch, prefix=prefix, verbose=args.verbose)
