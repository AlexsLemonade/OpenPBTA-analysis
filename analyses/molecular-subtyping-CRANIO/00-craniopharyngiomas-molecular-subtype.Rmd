...
snv_df <-
  read_tsv(file.path(root_dir, "data", "pbta-snv-consensus-mutation.maf.tsv.gz"))
```
...
Annotate the metadata table where the Kids_First_Biospecimen_ID matches either of the Tumor_Sample_Barcodes (TSBs) collected above
...
1. `CRANIO, To be classified` None of the above
Select the columns from the table that the PIs have requested
...
snv_subtype <-
  ...
  mutate(molecular_subtype =
           case_when(
             (BRAF.V600E & (as.numeric(age_at_diagnosis_days) >= age_of_adulthood)) & !(CTNNB1.Exon3 & (as.numeric(age_at_diagnosis_days) < age_of_adulthood)) ~ "CRANIO, PAP",
             !(BRAF.V600E & (as.numeric(age_at_diagnosis_days) >= age_of_adulthood)) & (CTNNB1.Exon3 & (as.numeric(age_at_diagnosis_days) < age_of_adulthood)) ~ "CRANIO, ADAM",
             (BRAF.V600E & (as.numeric(age_at_diagnosis_days) >= age_of_adulthood)) & (CTNNB1.Exon3 & (as.numeric(age_at_diagnosis_days) < age_of_adulthood)) ~ "CRANIO, ADAM;CRANIO, PAP",
             !(BRAF.V600E & (as.numeric(age_at_diagnosis_days) >= age_of_adulthood)) & !(CTNNB1.Exon3 & (as.numeric(age_at_diagnosis_days) < age_of_adulthood)) ~ "CRANIO, To be classified"
           )) %>%
