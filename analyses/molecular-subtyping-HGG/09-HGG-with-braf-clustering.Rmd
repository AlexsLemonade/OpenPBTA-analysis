...
## Identify sample that clusters with LGAT

```{r message = FALSE, warning = FALSE, fig.width = 16, fig.height = 10}
# Plot generation using `tumor_descriptor` as the variable supplied for shape.
# This variable was chosen because it was one of the variables in which the
# two samples where `HGAT_NO_HISTONE_BRAF` == "yes" differ.

plot_dimension_reduction(
  aligned_scores_df = stranded_tsne_aligned_scores,
  point_color = "subtype_class",
  point_size = "HGAT_NO_HISTONE_BRAF",
  point_shape = "tumor_descriptor",
  x_label = "t-SNE1",
  y_label = "t-SNE2"
) +
  scale_color_manual(values = c("#0a7fb2", "#799d10", "#F3E500"))
...
# According to the plots above, there is one sample that should be reclassified.
# This sample is an initial CNS Tumor that is currently classified as HGAT with
# no histone mutation but with a `BRAF V600E` mutation. We will use this
# information to isolate this sample.

stranded_tsne_aligned_scores %>%
  filter(tumor_descriptor == "Initial CNS Tumor", subtype_class == "HGAT_NO_HISTONE_BRAF") %>%
