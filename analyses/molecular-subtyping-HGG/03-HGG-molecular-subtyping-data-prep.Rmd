...
snv_df <-
  readr::read_tsv(file.path(input_dir, "hgg_snv_maf.tsv"))
```
...
snv_df <- snv_df %>%
  dplyr::mutate(
    IDH1.R132H = dplyr::case_when(Hugo_Symbol == "IDH1" &
                                    HGVSp_Short == "p.R132H" ~ "Yes",
                                  TRUE ~ "No")
  ) %>%
  dplyr::select(sample_id, IDH1.R132H)
...
final_df <- final_df %>%
  dplyr::left_join(snv_df, by = "sample_id") %>%
  dplyr::distinct()
