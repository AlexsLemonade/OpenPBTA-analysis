...
sv_df_filtered <- snv_df %>%
  ...
  dplyr::mutate(
    ACVR1_mutated = paste(sort(unique(
      dplyr::case_when(stringr::str_detect(Hugo_Symbol, "ACVR1") ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    TP53_mutated = paste(sort(unique(
      dplyr::case_when(stringr::str_detect(Hugo_Symbol, "TP53") ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    ATRX_mutated = paste(sort(unique(
      dplyr::case_when(stringr::str_detect(Hugo_Symbol, "ATRX") ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    FGFR1_mutated = paste(sort(unique(
      dplyr::case_when(stringr::str_detect(Hugo_Symbol, "FGFR1") ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    SETD2_mutated = paste(sort(unique(
      dplyr::case_when(stringr::str_detect(Hugo_Symbol, "SETD2") ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    TERT_mutated = paste(sort(unique(
      dplyr::case_when(stringr::str_detect(Hugo_Symbol, "TERT") ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    IDH1_mutated = paste(sort(unique(
      dplyr::case_when(stringr::str_detect(Hugo_Symbol, "IDH1") ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    CIC_mutated = paste(sort(unique(
      dplyr::case_when(stringr::str_detect(Hugo_Symbol, "CIC") ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    FUBP1_mutated = paste(sort(unique(
      dplyr::case_when(stringr::str_detect(Hugo_Symbol, "FUBP1") ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    DAXX_mutated = paste(sort(unique(
      dplyr::case_when(stringr::str_detect(Hugo_Symbol, "DAXX") ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    P73_AS1_mutated = paste(sort(unique(
      dplyr::case_when(stringr::str_detect(Hugo_Symbol, "P73-AS1") ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", ")
  ) %>%
