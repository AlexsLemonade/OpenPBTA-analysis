...
metadata <- metadata %>%
  ...
  dplyr::mutate(
    age_at_diagnosis_years = as.numeric(age_at_diagnosis_days)/365
    ) %>%
  dplyr::select(
    sample_id,
...
final_df <- hgg_df %>%
  dplyr::select(
    -Kids_First_Participant_ID,-Kids_First_Biospecimen_ID,-disease_type_new,-disease_type_reclassified
  ) %>%
...
genes_cn_df <- cn_df %>%
  ...
  dplyr::rename(PDGFRA_focal_status = PDGFRA,
...
sv_df_filtered <- cn_df %>%
  ...
  dplyr::mutate(
    ACVR1_mutated = paste(sort(unique(
      dplyr::case_when(gene_symbol == "ACVR1" ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    TP53_mutated = paste(sort(unique(
      dplyr::case_when(gene_symbol == "TP53" ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    ATRX_mutated = paste(sort(unique(
      dplyr::case_when(gene_symbol == "ATRX" ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    FGFR1_mutated = paste(sort(unique(
      dplyr::case_when(gene_symbol == "FGFR1" ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    SETD2_mutated = paste(sort(unique(
      dplyr::case_when(gene_symbol == "SETD2" ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    TERT_mutated = paste(sort(unique(
      dplyr::case_when(gene_symbol == "TERT" ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    IDH1_mutated = paste(sort(unique(
      dplyr::case_when(gene_symbol == "IDH1" ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    CIC_mutated = paste(sort(unique(
      dplyr::case_when(gene_symbol == "CIC" ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    FUBP1_mutated = paste(sort(unique(
      dplyr::case_when(gene_symbol == "FUBP1" ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", "),
    DAXX_mutated = paste(sort(unique(
      dplyr::case_when(gene_symbol == "DAXX" ~ "Yes",
                       TRUE ~ "No")
    )), collapse = ", ")
  ) %>%
...
gistic_df <- gistic_df %>%
  ...
  dplyr::group_by(sample_id) %>%
  dplyr::mutate(
    chr_1p_loss = dplyr::case_when(`1p` == -1 ~ "Yes", TRUE ~ "No"),
    chr_19q_loss = dplyr::case_when(`19q` == -1 ~ "Yes", TRUE ~ "No"),
    chr_7_loss = dplyr::case_when(`7p` == -1 |
                                    `7q` == -1 ~ "Yes", TRUE ~ "No"),
    chr_10_loss = dplyr::case_when(`10p` == -1 |
                                     `10q` == -1 ~ "Yes", TRUE ~ "No")
  ) %>%
