...
cn_df %>%
  ...
  select(-cytoband) %>%
  filter(gene_symbol %in% c("PDGFRA", "PTEN", "MYCN")) -> cn_df_genes

# Select only the ID information, for use in joining below
cn_df_genes %>%  dplyr::select(sample_id, Kids_First_Biospecimen_ID, Kids_First_Participant_ID) -> cn_df_genes_ids
...
```{r}
final_hgg_cn_df <- inner_join(cn_df_genes_status,
                              gistic_status_df,
