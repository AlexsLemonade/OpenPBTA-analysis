...
```{r}
no_evidence_ids <- setdiff(missing_bsids, starfusion_missing)

fusion_df <- fusion_df %>%
  # First add in the instances where we have no evidence of these fusions,
  # these will be IDs that are present in the STARFusion file
  bind_rows(data.frame(Sample = no_evidence_ids)) %>%
  replace(is.na(.), "None") %>%
  # For the IDs that are not in the STARFusion file, leave the fusion values
  # as NA
  bind_rows(data.frame(Sample = starfusion_missing))
```

```{r}
rm(arriba_df, putative_oncogenic_df, starfusion_df, starfusion_missing,
   missing_bsids, no_evidence_ids)
```
