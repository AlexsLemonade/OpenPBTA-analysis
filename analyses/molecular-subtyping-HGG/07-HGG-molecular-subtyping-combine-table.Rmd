...
```
The duplicates are mostly from multiple Solid tumor samples per sample_id or there are non-tumor ( derived cell-line ) samples for sample_id .The `cell_line_composition` seems to be able to differentiate each cell line samples. 
...
Let's add `cell_line_composition`column to merged dna and rna files cell-lines appropriately 
