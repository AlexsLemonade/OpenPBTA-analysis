...
dna_df <- mutation_df %>%
  ...
  dplyr::mutate(parent_aliquot_id = gsub("[.]WGS|WXS|Panel","",parent_aliquot_id))
```
...
checking if they have unqiue `parent_aliquot_id`. The parent_aliquot_id is different between cell-lines and solid tumor. For 7316-195 each cell line had different parent_aliquot_id 7316-195-T-203029.WGS, 7316-195-T-202952.WGS
...
histologies_df %>%
  ...
  group_by(sample_id) %>%
  arrange(sample_id) %>%
  dplyr::select(Kids_First_Biospecimen_ID,sample_id,parent_aliquot_id,composition) %>%
  summarize(parent_aliquot_id = paste(parent_aliquot_id, collapse = ", "),
...
rna_df <- histologies_df %>%
  ...
  inner_join(rna_df) %>%
  dplyr::mutate(parent_aliquot_id = gsub("[.]RNA-Seq","",parent_aliquot_id))
```
...
histologies_df %>%
  ...
  summarize(RNA_library = paste(RNA_library, collapse = ", "),
            parent_aliquot_id = paste(parent_aliquot_id, collapse = ", "),
            composition =  paste(composition, collapse = ", "))
```
...
molecular_subtype_table <- all_data_df %>%
  select(Kids_First_Participant_ID, sample_id,parent_aliquot_id,
