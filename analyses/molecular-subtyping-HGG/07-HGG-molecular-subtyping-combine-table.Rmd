...
h3_k28_df_biospecimen <- all_data_df %>%
  ...
  mutate(molecular_subtype = case_when(
    BRAF_V600E == "p.V600E" ~ "BRAF_V600E_mutation",
    TRUE ~ "HGG_H3_K28"
  )) %>%
  select(-BRAF_V600E)
```
...
We don't see the _IDH1_ mutations we would expect and we do see _TP53_ and _ATRX_ mutations, so we will leave this off as well for the moment.
...
head(molecular_subtype_table, n = 10)
```
