...
histologies_df %>%
  ...
  group_by(sample_id) %>%
  arrange(sample_id) %>%
  dplyr::select(Kids_First_Biospecimen_ID,sample_id,composition) %>%
  summarize(
...
Checking for duplicate rna sample_ids
...
Checking if adding cell-line composition type can uniquely match DNA to RNA

```{r}

cell_line_composition <- read_tsv("input/cell-line-composition.tsv",col_types = 
...
cell_line_composition %>%
  ...
  summarize(cell_line_composition = paste(cell_line_composition, collapse = ", "),
            composition =  paste(composition, collapse = ", "))

```
`cell_line_composition` seems to be able to differentiate each cell line so let's add the column to merged dna and rna files
