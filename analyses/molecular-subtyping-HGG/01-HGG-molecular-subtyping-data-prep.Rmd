...
tmb_df_filtered <- tmb_df %>%
  dplyr::filter(
    Hugo_Symbol == "H3F3A" &
      ...
      Hugo_Symbol == "H3F3A" &
      HGVSp_Short == "p.G35R" |
      Hugo_Symbol == "H3F3A" & HGVSp_Short == "p.G35V" |
      Hugo_Symbol == "HIST1H3B" & HGVSp_Short == "p.K28M" |
      Hugo_Symbol == "IDH1" & HGVSp_Short == "p.R132H" |
      Hugo_Symbol == "IDH1" & HGVSp_Short == "p.R172" |
      Hugo_Symbol == "BRAF" & HGVSp_Short == "p.V600E" |
      Hugo_Symbol %in% H3_K28 |
      Hugo_Symbol %in% H3_G35
  ) %>%
  dplyr::select(Tumor_Sample_Barcode, Hugo_Symbol, HGVSp_Short) %>%
  dplyr::mutate(Present_HGG_mutations = dplyr::case_when(
    Hugo_Symbol == "H3F3A" & HGVSp_Short == "p.K28M" ~ "H3F3A-K28M",
    Hugo_Symbol == "H3F3A" & HGVSp_Short == "p.G35R" ~ "H3F3A-G35R",
    Hugo_Symbol == "H3F3A" & HGVSp_Short == "p.G35V" ~ "H3F3A-G35V",
    Hugo_Symbol == "HIST1H3B" & HGVSp_Short == "p.K28M" ~ "HIST1H3B-K28M",
    Hugo_Symbol == "IDH1" & HGVSp_Short == "p.R132H" ~ "IDH1-R132H",
    Hugo_Symbol == "IDH1" & HGVSp_Short == "p.R172" ~ "IDH1-R172",
    Hugo_Symbol == "BRAF" & HGVSp_Short == "p.V600E" ~ "BRAF-V600E",
    TRUE ~ "NA"
  ))
...
# Display `cn_df_filtered`
cn_df_filtered %>%
  head(n = 15)

# Create data.frame with the filtered CN data.frame and the filtered tmb 
# data.frame
final_df <- tmb_df_filtered %>%
  dplyr::left_join(cn_df_filtered, by = "sample_id")
```
...
final_df <- final_df %>%
  ...
  dplyr::left_join(select_metadata, by = "sample_id") %>%
  dplyr::group_by(sample_id) %>%
  dplyr::mutate(
    ...
    Present_HGG_mutations = paste(sort(unique(
      Present_HGG_mutations
    )), collapse = ", "),
    Chromosomal_status = paste(sort(unique(Chromosomal_status)), collapse = ", ")
  ) %>%
  dplyr::select(
    -c(
      "Tumor_Sample_Barcode",
  ...
  dplyr::filter(Present_HGG_mutations != "BRAF-V600E") %>% # BRAF V600E is exclusive to LGG
  dplyr::distinct()
