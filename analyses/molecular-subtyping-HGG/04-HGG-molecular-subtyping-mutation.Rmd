...
We'll take a similar approach to the [`tp53-nf1-score`](../tp53-nf1-score/) module and use functions from the [`snv-callers`](../snv-callers/) module.
...
Samples that were classified as HGAT in the `short_histology` column of `pbta-histologies.tsv` and those with one of these defining lesions were included in the subset files (see: [`02-HGG-molecular-subtyping-subset-files.R`](./02-HGG-molecular-subtyping-subset-files.R)).
