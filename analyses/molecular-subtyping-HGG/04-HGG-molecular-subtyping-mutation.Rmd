...
For all other genes, we'll restrict to the coding sequences and drop silent mutations.
...
_Note that some samples that did not get included in the subset files, namely those that `disease_type_new == "Brainstem glioma- Diffuse intrinsic pontine glioma"`, have R132H mutations.
...
coding_summary_df <- coding_snv_df %>% 
  dplyr::select(Tumor_Sample_Barcode,
                Hugo_Symbol) %>%
  # For each tumor biospecimen, get a comma sepearted
...
We'll make a `data.frame` that indicates the Variant Classification of any _TERT_ mutation in a tumor biospecimen.
...
And now a `data.frame` that contains the specific IDH1 mutations.
