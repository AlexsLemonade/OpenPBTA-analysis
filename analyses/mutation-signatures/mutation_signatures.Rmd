...
```{r}
ind_samples <- readr::read_tsv(file.path(data_dir, 
...
cosmic_sigs_df <- calc_mut_per_sig(sample_sigs_cosmic,
                                   ...
                                   maf_df = maf) %>% 
  readr::write_tsv(results_dir, "cosmic_signatures_results.tsv")
cosmic_sigs_df
```
...
nature_sigs_df <- calc_mut_per_sig(sample_sigs_nature,
                                   ...
                                   wxs_exome_size = wxs_size, 
                                   maf_df = maf) %>% 
  readr::write_tsv(results_dir, "nature_signatures_results.tsv")
...
```{r}
bubble_matrix_plot(nature_sigs_df, label = "Alexandrov et al, 2013")
```
