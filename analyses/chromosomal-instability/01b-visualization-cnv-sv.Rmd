...
cnv_df <- data.table::fread(
  ...
  dplyr::mutate(
    ...
    direction = factor(direction, levels = c("del", "neutral", "amp"))
  )
```
...
Here we will add a column to `cnv_df` that color codes based on the `direction` 
column which has three categories. 

```{r}
# Let's determine how many levels this factor column has
n_levels <- length(levels(cnv_df$direction))
...
names(palette_key) <- levels(cnv_df$direction)
...
cnv_df <- cnv_df %>%
  ...
  dplyr::mutate(fac_color_key = dplyr::recode(direction, !!!palette_key))
```
...
circos_map_plot(
  ...
  add_track = FALSE,
  samples_col = "ID",
  sample_names = "BS_007JTNB8", # One sample here
  chr_col = "chrom",
  start_col = "loc.start",
  end_col = "loc.end",
  y_val = "copy.num",
...
circos_map_plot(
  ...
  samples_col = "ID",
  sample_names = "BS_00TRPEQX", # Second sample here. 
  chr_col = "chrom", #
  start_col = "loc.start",
  end_col = "loc.end",
  y_val = "copy.num",
