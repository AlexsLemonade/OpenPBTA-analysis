...
`circos_map_transloc`: Given a data.frame with two sets of coordinates, map the links between those coordinates on a new or exisitng circos plot. 
...
- `track_height`: a number between 0 and 1 that designates height, default is `.15`. 1 = the full diameter of the circos plot.  
...
transloc_df  <- transloc_df %>% 
  ...
  dplyr::inner_join(dplyr::filter(transloc_df, pair_num == 1), by = "match_id", 
                    suffix = c("1", "2"))
```
...
circos_map_plot(df = cnv_df, 
                ...
                palette = "Accent") # This is the color brewer palette we'll use. 
```
...
circos_map_plot(df = cnv_df, 
                ...
                y_val = "copy.num",
                track_height = .15, 
                type = "line", # type is set to point
...
circos_map_plot(df = cnv_df, 
                ...
                type = "line", 
                colour_key = "ID", 
                palette = "Set1"); # We'll change the color though for fun
# Add rect layer 
circos_map_plot(df = cnv_df, 
