...
palettes <- readr::read_rds(file.path(figures_dir, "palettes", "hex_color_palettes.rds"))

# Read in the by sample palette table
histology_col_by_sample <- readr::read_tsv(
  file.path(figures_dir, "palettes", "histology_color_by_sample.tsv"))
```
...
chr_colors <- rep(palettes$binary_col_palette, length.out = length(unique(chrs)))
