...
```{r}
intersection_heatmap_file <- file.path(plots_dir, paste0("intersection_breaks_heatmap.png"))
cnv_heatmap_file <- file.path(plots_dir, paste0("cnv_breaks_heatmap.png"))
sv_heatmap_file <- file.path(plots_dir, paste0("sv_breaks_heatmap.png"))
```
...
chr_colors  <-  as.character(dplyr::recode(chrs, !!!chr_colors))
...
# Make the same order as the data.frame
histologies_colors <-  as.character(dplyr::recode(histologies$short_histology, !!!histologies_colors))

# Make the names 
names(histologies_colors) <- as.character(rownames(histologies))
```
...
breaks_heatmap <- function(binned_counts_df, 
  ...
  #
  # Args:
  # binned_counts_df: a data.frame with the binned counts for each sample.
  ...
  hist_annot <- ComplexHeatmap::HeatmapAnnotation(
    ...
    col = list(histologies = histologies_colors), 
    which = "row",
    show_annotation_name = FALSE
  )
  # Plot on a heatmap
