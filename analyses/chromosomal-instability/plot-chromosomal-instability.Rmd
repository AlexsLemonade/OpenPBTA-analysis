...
data that was formatted by `01-setup-breakpoint_data.R` and mapping chromosomal 
break density.
...
```{r}
breaks_list <- readr::read_rds(file.path(scratch_dir, "breaks_lists.RDS"))
```

Get a vector of the biospecimen IDs. 

```{r}
common_samples <- unique(breaks_list$common_breaks$samples)
```
...
```{r}
# Save to an RDS file
readr::write_rds(sample_densities, 
...
```{r}
# Save to an RDS file
readr::write_rds(group_densities, 
