...
```{r}
breaks_list <- readr::read_rds(file.path(output_dir, "breaks_lists.RDS"))
```
...
sample_densities <- lapply(common_samples, function(sample_id) {
  all_break_density(breaks_list = breaks_list, 
                    sample_id = sample_id, 
                    return_vector = TRUE) # We only want the total counts, 
...
```{r}
# Turn intersection data into its own data.frame
intersection_df <- purrr::map(sample_densities, "intersection_of_breaks") %>% 
  dplyr::bind_rows(.id = "chr") %>% 
  readr::write_tsv(file.path(output_dir, 
                            "intersection_binned_breakpoint_counts.tsv"))

# Same for CNV 
cnv_df <- purrr::map(sample_densities, "cnv_breaks") %>% 
  dplyr::bind_rows(.id = "chr") %>% 
  readr::write_tsv(file.path(output_dir, 
                            "cnv_binned_breakpoint_counts.tsv"))

# Same for SV
sv_df <- purrr::map(sample_densities, "sv_breaks") %>% 
  dplyr::bind_rows(.id = "chr") %>% 
  readr::write_tsv(file.path(output_dir, 
                            "sv_binned_breakpoint_counts.tsv"))
```
...
group_densities <- lapply(tumor_types, function(tumor_type) {
  ...
  all_break_density(breaks_list = breaks_list,
                    ...
                    return_vector = FALSE) # We want the whole GenomicRanges object
})
...
readr::write_rds(
  group_densities,
  file.path(output_dir, "histology_breakpoint_binned_counts.RDS")
)
```
