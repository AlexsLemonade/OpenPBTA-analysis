...
```{r}
histologies <- 
  data.frame(Kids_First_Biospecimen_ID = rownames(common_total_counts)) %>%
...
```{r}
chr_labels <- sample_densities$BS_CGXTFM67$common_density@seqnames
chr_labels <- paste0("chr", as.factor(chr_labels))
#chr_labels <- factor(chr_labels, levels = c(paste0("chr", 1:22)))
...
chr_annot <- ComplexHeatmap::HeatmapAnnotation(df = data.frame(chr_labels), col = list(chr_labels = c(chr_colors)), 
                                               show_legend = FALSE,
                                               gap = grid::unit(3, "points"))
```


### Common density heatmap

set up the data
```{r}
# Pull out the total_counts 
common_total_counts <- lapply(sample_densities, function(granges_list) {
  granges_list$common_density@elementMetadata@listData$total_counts
})
# Make into a data.frame
common_total_counts <- dplyr::bind_rows(common_total_counts) %>%
  t()

# Log2 transform it
common_total_counts <- log2(0.01 + common_total_counts)

colnames(common_total_counts) <- chr_labels
```

Make a color function. 

```{r}
col_fun <- circlize::colorRamp2(c(min(common_total_counts), 
                                  mean(common_total_counts), 
                                  max(common_total_counts)), 
                                c("deepskyblue", "black", "yellow"))
```
...
```{r}
# Plot on a heatmap
common_heatmap <- ComplexHeatmap::Heatmap(common_total_counts, 
                                          name = "histology", 
                                          col = col_fun, 
                                          row_title = "Biospecimens", 
