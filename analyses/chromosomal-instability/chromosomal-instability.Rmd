...
densities <- lapply(common_samples, function(sample_id) {
  ...
  all_densities <- GenomicRanges::GRangesList(common_density, 
                                              cnv_density, 
                                              sv_density)
  # Uncompress this into a normal GRanges object
  all_densities <- unlist(all_densities)
  
  # Add dataset of origin label in listData
  all_densities@elementMetadata@listData$dataset <- 
    rep(c("common", "cnv_only", "sv_only"), each = 2897)
...
names(common_density) <- common_samples
```

Get CNV breakpoint density

```{r}
cnv_density <- lapply(common_samples, function(sample_id) {
  break_density(cnv_breaks, 
                sample_id = sample_id,
                window_size = 1e6,
                chr_sizes_list = chr_sizes_list)
})
...
# Bring along the sample IDs
names(cnv_density) <- common_samples
```

Get SV breakpoint density

```{r}
sv_density <- lapply(common_samples, function(sample_id) {
  break_density(sv_breaks, 
                sample_id = sample_id,
                window_size = 1e6,
                chr_sizes_list = chr_sizes_list)
})
...
# Bring along the sample IDs
names(sv_density) <- common_samples
```
