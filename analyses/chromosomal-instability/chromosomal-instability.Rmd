...
```{r, results='hide'}
lapply(densities, function(granges_list){
...
# Get a list of the tumor_types
tumor_types <- unique(metadata$short_histology)
...
group_densities <- lapply(tumor_types, function(tumor_type) {
  ...
  sample_ids <- metadata %>% 
    dplyr::filter(tumor_type %in% short_histology)
