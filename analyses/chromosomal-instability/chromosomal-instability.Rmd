...
sv_data <- lapply(sv_files, data.table::fread, colClasses = "character", 
                  data.table = FALSE)
...
sv_df <- dplyr::bind_rows(sv_data, .id = "sample_names") %>%
```
...
sv_breaks <- sv_df %>%
  # Gather the chromosomal coordinate variables
  tidyr::gather(key, value, -sample_names, -V1, -homolen, -inserlen, -SVtype) %>%
  # Make the `1` and `2` variables identical so they can be made into one column
  dplyr::mutate(key = gsub("1|2", "", key)) %>%
  # There are repeat values so we need to make an ID column
  tibble::rowid_to_column() %>% 
  # Spread the values to their respective columns
  tidyr::spread(key, value)
```

Make a `GenomicRanges` object for SV data.

```{r}
sv_ranges <- GenomicRanges::GRanges(
    seqnames = sv_df$chrom1,
    ranges = IRanges::IRanges(
      start = as.numeric(sv_df$pos),
      end = as.numeric(sv_df$pos)
    ),
    mcols = sv_df
  )
```
