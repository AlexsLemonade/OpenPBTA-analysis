...
# Borrowing the custom plotting function from the `tmb-compare-tcga` analysis plots
source(file.path("..", "tmb-compare-tcga", "util", "sina_plot.R"))
```
...
```{r}
breaks_sina <- sina_plot(breaks_densities_df, 
          colour = "#630882", 
          x_axis = "short_histology", 
          y_axis = "breaks_density", 
          title = "Chromosomal Break Densities", 
          y_label = "Chromosomal breaks per Mb") + 
  # Adjust the background stripe size
    ggplot2::theme(
      panel.grid.major.x = ggplot2::element_line(colour = c("gray93", "white"), size = 12.75))

# Save the plot to a png
ggplot2::ggsave(file.path(plots_dir, "breaks_density_sina_plot.png"), 
                   plot = breaks_sina, width = 35, height = 20, unit = "cm")
```

Print from png so rendering is smoother
![Breaks Density Plot](./plots/breaks_density_sina_plot.png)
