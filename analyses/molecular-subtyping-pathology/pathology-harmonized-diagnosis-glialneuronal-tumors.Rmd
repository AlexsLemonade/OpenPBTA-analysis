...
-- | -- | -- | -- | -- | -- | -- 
Low-grade glioma/astrocytoma (WHO grade I/II) | NA, remove from LGAT module | contains "desmoplastic infantile astrocytoma" | Neuronal and mixed neuronal-glial tumor | GNT | Desmoplastic infantile astrocytoma, <subtype> | Desmoplastic infantile astrocytoma 
...
subtype_df <- histologies_df %>%
  filter(
    ...
    str_detect(str_to_lower(pathology_free_text_diagnosis),
               paste(lgat_path_dx_list$recode_path_free_text,
                     collapse = "|"))
  ) %>%
...
subtype_df <- compiled_subtypes_df %>%
  inner_join(subtype_df,
             by=c("Kids_First_Participant_ID", "sample_id", "Kids_First_Biospecimen_ID")) 
...
dia_df <- subtype_df %>%
  ...
  filter(str_detect(pathology_free_text_dx_lower,
                    lgat_path_dx_list$recode_path_free_text[1])) %>%
  
  mutate(
    integrated_diagnosis = if_else(
      # Set integrated diagnosis if sample is subtyped
      !is.na(molecular_subtype),
      gsub("GNT, ","Desmoplastic infantile astrocytoma, ",molecular_subtype), 
      NA_character_),
    # Set harmonized diagnosis
    harmonized_diagnosis = "Desmoplastic infantile astrocytoma",
...
glioneuronal_df <- subtype_df %>%
  ...
  filter(str_detect(pathology_free_text_dx_lower,
                    lgat_path_dx_list$recode_path_free_text[2])) %>%
  mutate(
    ...
    !is.na(molecular_subtype) ~ 
      gsub("GNT,","Glial-neuronal tumor NOS, ", molecular_subtype),
