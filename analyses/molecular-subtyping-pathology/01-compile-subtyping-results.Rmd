...
lgat_results_df <- bind_exp_strategies(lgat_results_df) %>%
  inner_join(select(histologies_df,
                    ...
                    Notes),
             by = "Kids_First_Biospecimen_ID") %>%
  mutate(integrated_diagnosis = case_when(grepl("LGG", molecular_subtype) ~ paste0("Low-grade glioma/astrocytoma, ", molecular_subtype),
                                          TRUE~ NA_character_),
         #remove "LGG, To be classified" from integrated dx
         integrated_diagnosis = ifelse(molecular_subtype == "LGG, To be classified", NA_character_, integrated_diagnosis),
         #delete LGG string from integrated dx
         integrated_diagnosis = case_when(grepl("LGG", integrated_diagnosis) ~ str_replace(integrated_diagnosis, "LGG, ", "")),
