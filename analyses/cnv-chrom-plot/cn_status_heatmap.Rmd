...
```{r}
#TODO: Remove this when this is split into two panels
small_group <- metadata %>% 
  dplyr::filter(Kids_First_Biospecimen_ID %in% bin_calls_df$biospecimen_id) %>%
  dplyr::group_by(short_histology) %>%  
  dplyr::summarize(group_n = dplyr::n()) %>% 
  dplyr::filter(group_n < 2) %>% 
  dplyr::pull(short_histology)

metadata$short_histology[metadata$short_histology %in% small_group] <- "Other"
```

Add the sample color column.
