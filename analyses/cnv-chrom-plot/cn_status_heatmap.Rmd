...
coverage is a certain threshold larger than the other status' 
coverage (`frac_delta_threshold`).
...
bin_calls_list <- lapply(sample_ids,
  ...
  # Percent coverage difference for a bin to be called a particular status
  frac_delta_threshold = frac_delta_threshold
)
...
bin_indices <- names(pct_overlap)[which(pct_overlap > frac_uncallable)]
```
...
```{r}
color_key <- structure(c(divergent_col_palette, "#9932CC"), 
  names = c("loss", "neutral", "gain", "unstable")
)
```
