...
This is done by binning the genome and calculating the segment's coverage of the CNV consensus segments. 
A bin is declared a particular copy number status if that status's base pair coverage is a certain threshold percentage larger than the other statuses' coverage. 
...
bp_per_bin <- function(bin_ranges, status_ranges) {
  ...
  #   status_ranges:A GenomicRanges object to calculate what percent coverage of each bin. 
  #
  # Returns:
...
Function that determines the overall CN status of each genome bin. 
...
```{r}
seg_ranges <- seg_ranges[which(seg_ranges@ranges@width < length_max)]
```
...
```{r}
color_key <- structure(c("#ca0020", "#f7f7f7", "#0571b0"),
