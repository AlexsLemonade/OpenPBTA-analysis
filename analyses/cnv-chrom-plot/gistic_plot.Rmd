...
seg_data <- data.table::fread(file.path(input_dir, "pbta-cnv-cnvkit.seg.gz"), 
                              data.table = FALSE) %>% 
  # Calculate a ratio 
  dplyr::mutate(ratio = copy.num/seg.mean) %>% 
  # Join the histology column to this data 
  dplyr::inner_join(dplyr::select(metadata, 
...
ggplot2::ggplot(gistic_ranges) + 
  geom_bar(ggplot2::aes(y = score, fill = mcols.Type), scales = "free_x", space = "free_x") + 
...
### Log R Ratio plots by histology 
...
lapply(unique(seg_data$short_histology), function(histology_group) {
  ...
  # Only continue if there are at least 5 samples to plot
  if (nrow(hist_df) > 5) {
  ...
  # Map this on a plot
  ggplot2::ggplot(hist_ranges) + 
    geom_bar(ggplot2::aes(y = score, fill = mcols), scales = "free_x", space = "free_x") + 
    ggplot2::theme_classic() +
    ggplot2::theme(axis.text.x = ggplot2::element_text(size = 3, angle = 45, hjust = 1)) +
    colorblindr::scale_fill_OkabeIto(name = "Type") + 
  ...
  ggplot2::ggsave(filename = file.path(plots_dir, paste0(histology_group, "_plot.png")), 
                  width = 10, 
                  height = 3, 
                  units = "in")
  }
