...
```{r}
metadata <- metadata %>% 
  dplyr::mutate(molecular_subtype = dplyr::recode(sample_id, !!!key, .default = "NA")) %>%   dplyr::filter(molecular_subtype == "SHH")
...
```{r}
tp53_mut <- tp53_mut %>% 
  dplyr::inner_join(dplyr::select(metadata, "Kids_First_Participant_ID", "sample_id", "Kids_First_Biospecimen_ID"),
