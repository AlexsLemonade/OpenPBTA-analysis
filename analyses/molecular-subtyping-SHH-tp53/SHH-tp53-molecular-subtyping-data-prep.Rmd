...
tp53_status_table <- shh_metadata %>%
  ...
  dplyr::select("Kids_First_Participant_ID", "sample_id", "Kids_First_Biospecimen_ID", "experimental_strategy") %>%
  # Collapse Biospecimen ids for each sample
  dplyr::group_by(Kids_First_Participant_ID, sample_id) %>%
  dplyr::summarize(Kids_First_Biospecimen_IDs = paste(Kids_First_Biospecimen_ID, collapse = ", "), 
                   experimental_strategies = paste(experimental_strategy, collapse = ", ")) %>%
...
```{r}
readr::write_tsv(tp53_status_table, file.path(results_dir, "tp53-shh-samples-status.tsv"))
```
