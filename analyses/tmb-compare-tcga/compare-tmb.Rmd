...
```
Rscript -e "rmarkdown::render('analyses/tmb-calculations/compare-tmb.Rmd', 
                              clean = TRUE)"
```
...
Custom plot for plottting the TMB. 
...
sina_plot <- function(data, colour = NULL, title) {
  ...
  data %>%
    ggplot2::ggplot(ggplot2::aes(x = short_histology, y = log2(tmb))) +
...
We will only use the brain related TCGA projects, so we are specifying them here. 

```{r}
tcga_datasets <- c("LGG", "GBM", "PCPG")
```

Download and read each of these MAF files using `TCGAbiolinks`. 
...
Report the ratio of mutations that are within this window. 
...
```
All of these mutations are within this window, so we don't need to filter them out. 
...
tcga_plot <- sina_plot(tmb_tcga, colour = "darkred", title = "TCGA (Adult)") +
  ggplot2::theme(
...
pbta_plot <- sina_plot(tmb_pbta, colour = "blue", title = "PBTA")
