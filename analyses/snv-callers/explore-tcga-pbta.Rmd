...
```{r}
tmb_combo <- function(pbta_df = pbta_lancet, tcga_df = tcga_lancet) {
# Calculate coding only TMBs and write to file
  ...
  dplyr::bind_rows(list(pbta = pbta_coding_df, 
                        tcga = tcga_coding_df), .id = "dataset")  %>% 
  dplyr::mutate(dataset = as.factor(dataset)) %>%
    ggplot2::ggplot(ggplot2::aes(x = short_histology, color = dataset, 
                                 y = log10(tmb))) +
    ggforce::geom_sina(alpha = 0.3, maxwidth = 0.9) +
