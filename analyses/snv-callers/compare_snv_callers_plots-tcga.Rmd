...
Set up custom function for upset plot. 

```{r}
upset_plot <- function(detect_df, subset_vector = NULL) {
  ...
  #
  # Arguments: 
  # detect_df: a data frame, where each column represents a caller and contains a TRUE/FALSE 
  #            for whether a mutation was detected. 
  # subset_vector: (Optional) a vector to be supplied to dplyr::filter() that will subset the
  #                detect_df and create the upset plot from that subset.
  #
  ...
  # Plot it
  UpSetR::upset(UpSetR::fromList(detect_list), order.by = "freq")
}
```
...
detect_df <- all_caller_df %>% 
 ...
 dplyr::select(dplyr::starts_with("VAF_"), index) %>% 
 # Turn into detected or not
 dplyr::mutate_at(dplyr::vars(dplyr::starts_with("VAF_")), function(x) !is.na(x))
```
...
consensus_count <- detect_df %>%
  # Ignore index temporarily
  dplyr::select(-index) %>%
  # Get the number of callers that call each mutation
  dplyr::transmute(num = rowSums(.)) %>% 
...
```{r}
upsettr_plot <- upset_plot(detect_df)
```

Save this plot to a png

```{r}
png(file.path(plots_dir, "tcga-upset_plot.png"), width = 900, height = 900)
upsettr_plot
dev.off()
```
...
upset_plot(detect_df, subset_vector = transcripts)
```
...
upset_plot(detect_df, subset_vector = not_transcripts)
```
...
ggplot2::ggsave(plot = vaf_plot, 
                file.path(plots_dir, "tcga-vaf_distribution_plot.png"))
```
...
```{r}
ggplot2::ggsave(filename = file.path(plots_dir, "tcga-vaf_cor_matrix.png"), 
                plot = cor_vaf)
```
...
```{r}
# Save this plot
ggplot2::ggsave(file.path(plots_dir, "tcga-frac_base_change_plot.png"))
```
