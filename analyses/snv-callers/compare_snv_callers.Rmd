...
filter_vaf_files <- list.files(file.path(base_results_dir, "vaf_filter"),
  pattern = "_vaf.tsv$", recursive = TRUE,
  full.names = TRUE
)
...
vaf_list <- lapply(vaf_files, function(file) {
  readr::read_tsv(file) %>%
    # The file is large so we will cut down on its size by only keeping the columns we need
...
# Save this plot
png(file.path(compare_plots_dir, "upset_plot.png"), width = 900, height = 900)
upset
dev.off()
...
```{r}
# Save this plot
ggplot2::ggsave(file.path(compare_plots_dir, "upset_median_vaf_plot.png"))
```
...
```{r}
# Save this plot
ggplot2::ggsave(file.path(compare_plots_dir, "perc_base_change_plot.png"))
```
...
```{r}
# Save this plot
ggplot2::ggsave(file.path(compare_plots_dir, "vaf_violin_plot.png"))
```
...
```{r}
# Save this plot
ggplot2::ggsave(file.path(compare_plots_dir, "tmb_boxplot_histology_plot.png"))
```
...
```{r}
# Save this plot
ggplot2::ggsave(file.path(compare_plots_dir, "vaf_correlations_plot.png"))
```
...
```{r}
# Save this plot
ggplot2::ggsave(file.path(compare_plots_dir, "tmb_correlations_plot.png"))
```

## Genomic regions analysis

Calculate the percent of mutations in each type of genomic region per caller.

```{r}
# Summarize by the number of times each base change shows up in each category. 
perc_region_df <- perc_mutation_by_caller(region_annot_df, var = "type")
```

Plot the percent of mutations in each type of genomic region per caller.

```{r}
perc_region_df %>%
  ggplot2::ggplot(ggplot2::aes(x = reorder(type, -percent), y = percent)) +
  ggplot2::theme_classic() +
  ggplot2::geom_bar(
    position = "dodge", stat = "identity",
    ggplot2::aes(fill = caller)
...
```{r}
# Save this plot
ggplot2::ggsave(file.path(compare_plots_dir, "perc_genomic_regions_plot.png"))
```

Plot the VAF for each type of genomic region. 
...
```{r}
# Save this plot
ggplot2::ggsave(file.path(compare_plots_dir, "vaf_by_region_plot.png"))
```

## Save consensus mutation calls

Get an idea of how many mutations per group. 

```{r}
summary(as.factor(callers_per_mutation$caller_combo))
```

Get the mutation ids for these. 
...
consen_mutation <- vaf_df %>% 
  dplyr::filter(caller == "strelka2", 
                mutation_id %in% consen_mutations) %>% 
  readr::write_tsv(file.path(compare_results_dir, "conensensus_mutation.maf.tsv"))
```
...
```{r}
zip(
  file.path(compare_results_dir, "conensensus_mutation.maf.tsv.zip"),
  file.path(compare_results_dir, "conensensus_mutation.maf.tsv")
)
```
...
wgs_bed <- readr::read_tsv(file.path("..", "..", "data", "WGS.hg38.strelka2.unpadded.bed"),
                           col_names = FALSE)
wxs_bed <- readr::read_tsv(file.path("..", "..", "data", "WXS.hg38.100bp.padded.bed"),
                           col_names = FALSE)
...
tmb_df <- calculate_tmb(vaf_df,
  ...
  readr::write_tsv(file.path(compare_results_dir, "consensus_mutation_tmb.tsv"))
```
