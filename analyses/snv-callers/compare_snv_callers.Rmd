...
```{r}
perc_change_df %>%
  ggplot2::ggplot(ggplot2::aes(x = reorder(change, -percent), y = percent)) +
  ggplot2::theme_classic() +
  ggplot2::geom_violin(
...
vaf_df %>%
  ...
  ggforce::geom_boxplot(alpha = 0.05) +
  ggplot2::theme_classic() +
...
```{r}
# Save this plot
ggplot2::ggsave(file.path(compare_plots_dir, "vaf_sina_plot.png"))
```
...
```{r}
# Save this plot
ggplot2::ggsave(file.path(compare_plots_dir, "tmb_sina_plot.png"))
```
...
Do correlations amongst TMB statistics improve as we increase VAF? 
We need to re-calculate TMB statistics with various vaf_cutoffs
```{bash}
datasets=("strelka2" "mutect2" "lancet" "vardict")
vaf_cutoff=("0.05" "0.10" "0.15" "0.20")
...
for ((i=0;i<${#vaf_cutoff[@]};i++)); 
  ...
  do
    ...
    Rscript analyses/snv-callers/scripts/01-calculate_vaf_tmb.R \
      ...
      --maf data/testing/release-v5-20190924/pbta-snv-${datasets[$j]}.vep.maf.gz \
