...
num_of_callers <- all_caller_df %>% 
  # Isolate to VAFs but ignore VarDict)
  dplyr::select(dplyr::starts_with("VAF_"), -VAF_vardict) %>%
  # Remove rows that are all NAs
  dplyr::filter(apply(., 1, function(x) sum(is.na(x))) != 3) %>%
...
upsettr_plot <- callers_per_mutation %>%
  upset_plot() + 
  ggplot2::ylim(c(0, 6e+06)) # So we can see the number
...
callers_per_mutation %>% 
  ...
  upset_plot() + 
  ggplot2::ylim(c(0, 300000)) # So the label shows up
```
