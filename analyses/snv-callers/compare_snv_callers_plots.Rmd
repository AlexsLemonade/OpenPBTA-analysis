...
# Which should be part of the consensus
consensus_index <- which(apply(!is.na(vaf_df[, -3]), 1, sum) < 3)
...
```{r}
# Save this plot
ggplot2::ggsave(file.path(plots_dir, "upset_plot.png"))
```
...
perc_change_df <- all_caller %>% 
  dplyr::mutate(base_change = paste0(Allele, ">", Reference_Allele)) %>%
  # Case when only works for df not sql, so we will convert now
  as.data.frame() %>%
  dplyr::mutate(
  ...
  dplyr::select(-Allele, -Reference_Allele, -Chromosome, -base_change, 
                -Tumor_Sample_Barcode, -Start_Position) %>%
  # Make this long form
  ...
  dplyr::filter(!grepl("G>G|A>A|N>A|N>T|T>N|A>N", change), 
                !is.na(change)) %>%
  dplyr::mutate(
...
```{r}
GGally::ggpairs(vaf_df %>% dplyr::select(-VAF_consensus), 
