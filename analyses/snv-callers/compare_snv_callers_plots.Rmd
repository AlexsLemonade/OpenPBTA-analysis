...
Make a data.frame with only the VAFs. 

```{r}
vaf_df <- all_caller_df %>% 
 dplyr::select(dplyr::starts_with("VAF_")) %>%
 # The consensus VAF will be what's reported for strelka
...
```{r}
consensus_count <- vaf_df %>%
  # Ignore VarDict and consensus
  dplyr::select(-VAF_vardict, -VAF_consensus) %>%
...
vaf_df$VAF_consensus[not_consensus] <- NA
```

Make this into long form. 
...
upsettr_plot <- callers_per_mutation %>%
  upset_plot() + 
  ggplot2::ylim(c(0, 7e+06)) # So we can see the number
