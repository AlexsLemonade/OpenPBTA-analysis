...
params:
  project_acronym:
    label: "project acronymn to use as prefix for output files"
    value: kfnbl
    input: string
...
Rscript -e "rmarkdown::render('analyses/snv-callers/compare_snv_callers_plots.Rmd', 
                              output_file = 'compare_snv_callers_plots-<project acronym>',
			      params = list(project_acronym = '<project acronym>'), 
			      clean = TRUE)"
```
...
plots_dir <- file.path("plots", paste0(params$project_acronym, "-caller-comparison"))
...
upset_png(detect_mat, 
          plot_file_path = file.path(plots_dir, paste0(params$project_acronym, "-upset-plot.png")))
```

![Upset plot - all data](plots/`r paste0(params$project_acronym, "-caller-comparison")`/`r paste0(params$project_acronym, "-upset-plot.png")`)
...
upset_png(detect_mat, 
          plot_file_path = file.path(plots_dir, paste0(params$project_acronym, "-upset-plot-transcipts-only.png")), 
          subset_vector = transcripts)
```

![Upset plot - transcripts only](plots/`r paste0(params$project_acronym, "-caller-comparison")`/`r paste0(params$project_acronym, "-upset-plot-transcipts-only.png")`)
...
upset_png(detect_mat,
          plot_file_path = file.path(plots_dir, paste0(params$project_acrony, "-upset-plot-not-transcipts-only.png")), 
          subset_vector = not_transcripts)
```

![Upset plot - non transcripts only](plots/`r paste0(params$project_acronym, "-caller-comparison")`/`r paste0(params$project_acronym, "-upset-plot-not-transcipts-only.png")`)
...
ggplot2::ggsave(plot = vaf_plot, 
                file.path(plots_dir, paste0(params$project_acronym, "-vaf_distribution_plot.png")))
```
...
```{r, warning = FALSE}
ggplot2::ggsave(filename = file.path(plots_dir, paste0(params$project_acronym, "-vaf_cor_matrix.png")), 
                plot = cor_vaf)
```
...
```{r}
# Save this plot
ggplot2::ggsave(file.path(plots_dir, paste0(params$project_acronym, "-frac_base_change_plot.png")))
```
...
```{r}
# Save this plot
ggplot2::ggsave(file.path(plots_dir, paste0(params$project_acronym, "-variant_classification_plot.png")))
```
