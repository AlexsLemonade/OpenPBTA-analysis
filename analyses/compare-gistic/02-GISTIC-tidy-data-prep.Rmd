...
lgat_merged_calls_table <- prepare_gene_level_gistic(
  ...
  lgat_amp_genes_file,
  lgat_del_genes_file,
  output_filepath = file.path("results", "lgat_gistic_gene_cn_status_table.tsv.gz")
)

lgat_merged_calls_table %>%
  dplyr::filter(detection_peak == "Amplification Peak 3 - CN values")
```
...
cohort_merged_calls_table <- prepare_gene_level_gistic(
  ...
  output_filepath = file.path("results", "cohort_gistic_gene_cn_status_table.tsv.gz")
)
...
lgat_cytoband_status_table <- prepare_cytoband_level_gistic(
  lgat_all_lesions_file,
  output_filepath = file.path("results", "lgat_gistic_cytoband_cn_status_table.tsv.gz")
)
...
cohort_cytoband_status_table <-
  prepare_cytoband_level_gistic(
    cohort_all_lesions_file,
    output_filepath = file.path("results", "cohort_gistic_cytoband_cn_status_table.tsv.gz")
  )
