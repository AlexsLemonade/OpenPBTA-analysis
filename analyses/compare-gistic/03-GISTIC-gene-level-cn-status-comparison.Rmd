...
tally_calls <- function(gistic_calls_df,
  ...
  #   output_filepath: the string of file path to save the output table to
  #
  # Return:
  ...
  final_df <- gistic_calls_df %>%
    ...
    inner_join(focal_calls_df,
              by = c("Kids_First_Biospecimen_ID" = "biospecimen_id", "gene" = "gene_symbol"))
  ...
  final_loss_df <- final_df %>%
    group_by(gene, detection_peak, loss) %>%
    tally()
  
  final_gain_df <- final_df %>%
    group_by(gene, detection_peak, gain) %>%
    tally()
  
  final_amplification_df <- final_df %>%
    group_by(gene, detection_peak, amplification) %>%
    tally()
  
  final_neutral_df <- final_df %>%
    group_by(gene, detection_peak, neutral) %>%
    tally()
  ...
  if (focal_or_gistic == "focal") {
    ...
    final_df <- final_loss_df %>%
      inner_join(final_gain_df, by = c("gene", "detection_peak")) %>%
  ...
  if (output_filepath == file.path("results", "cohort_gene_level_focal_cn_counts.tsv")) {
    # We join the amplification data separately because the LGAT focal data
    # do not contain amplification calls
    final_df <- final_df %>%
      inner_join(final_amplification_df,
                 by = c("gene", "detection_peak")) %>%
      # Filter out non-focal calls
      filter(amplification == "focal") %>%
      mutate(amplification = n) %>%
      select(-n)
  }
  
  # Save final table
  write_tsv(final_df, output_filepath)
  
  # Display final table -- arrange by detection peak
  final_df %>%
...
tally_calls(
  ...
  output_filepath = file.path("results", "lgat_gene_level_focal_cn_counts.tsv")
)
...
tally_calls(
  ...
  output_filepath = file.path("results", "lgat_gene_level_gistic_cn_counts.tsv")
)
```
...
tally_calls(
  ...
  focal_calls_df = consensus_focal_cn_calls,
  focal_or_gistic = "focal",
  output_filepath = file.path("results", "cohort_gene_level_focal_cn_counts.tsv")
)
...
tally_calls(
  ...
  focal_calls_df = consensus_focal_cn_calls,
  focal_or_gistic = "gistic",
  output_filepath = file.path("results", "cohort_gene_level_gistic_cn_counts.tsv")
)
```
