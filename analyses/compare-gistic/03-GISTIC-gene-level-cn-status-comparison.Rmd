...
tally_calls <- function(gistic_calls_df,
  ...
  final_df <- final_df %>%
    ...
    # We are removing some fields here -- the `distinct` function is removing
    # the resulting duplicated rows
    distinct()
  ...
  if (output_filename == "cohort_gene_level_focal_cn_counts.tsv") {
...
consensus_focal_cn_calls <- consensus_focal_cn_calls %>%
  ...
  # We are selecting only the fields we need from this data.frame -- the
  # `distinct` function removes duplicated rows left due to the removal of
  # the other fields
  distinct()
```
