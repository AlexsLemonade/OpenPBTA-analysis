...
palette_mapping_df <- histologies_df %>%
  ...
  select(Kids_First_Biospecimen_ID,
         ...
         molecular_subtype)  %>%
  # Add schwannoma & neurofibroma for plotting
  mutate(molecular_subtype = case_when(cancer_group == "Schwannoma" ~ "Schwannoma",
                                       cancer_group == "Neurofibroma Plexiform" ~ "Neurofibroma",
...
```{r cancer_groups_celltypes_plot, fig.width = 8, fig.height = 6}
...
# Now, filter to relevant samples and remove uncharacterized fractions
quantiseq_subset2 <- quantiseq %>%
  left_join(palette_mapping_df, by = c("sample" = "Kids_First_Biospecimen_ID")) %>%
  filter(cancer_group %in% cg_of_interest & cell_type != "uncharacterized cell" & !is.na(cancer_group))

cg_plot <- ggplot(quantiseq_subset2) +
  ...
  facet_wrap(~cancer_group, ncol = 6) +
...
ggsave(cg_celltypes_plot_file, cg_plot, width = 8, height = 6)
```
...
pdl1_distributions_plot <- ggplot(pdl1_cd8) + 
  ...
  labs(x = "Molecular subtype of sample",
       y = "Log2 expression (FPKM) of PDL1") +
  ggpubr::theme_pubr() + 
  theme(axis.text.x = element_text(hjust = 1, 
                                   size = rel(0.6), 
                                   angle = 90)) 
