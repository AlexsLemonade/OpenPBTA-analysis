...
#### Array of groups we want to visualize (top 15 groups with highest N samples?)
...
#cancer_groups_of_interest <- c("Other high-grade gliomas", 
...
cg_of_interest <- palette_mapping_df %>%
  filter(!is.na(cancer_group_display), cancer_group_display != "Other") %>%
...
quantiseq_subset_cg <- quantiseq_gather %>%
  ...
  left_join(palette_mapping_df, by = c("sample" = "Kids_First_Biospecimen_ID")) %>%
  filter(cancer_group_display %in% cg_of_interest)
...
ggsave(cg_celltypes_plot_file, cg_plot, width = 11, height = 3.5)
```
...
ggsave(subtypes_celltypes_plot_file, subtypes_celltypes_plot, width = 13, height = 4)
```
...
ggsave(cd8_cd4_ratio_plot_file, ratio_plot, width = 2.75, height = 4)
