...
```{r}
subset_focal_cn_df <- 
  data.table::fread(file.path(input_dir, "chordoma-only_seg_annotated_cn_autosomes.tsv.gz"))
```
...
chordoma_copy <- chordoma_copy %>%
  ...
  inner_join(select(subset_metadata,
