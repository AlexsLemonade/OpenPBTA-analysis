...
plot_tumor_violins <- function(df, display_name, display_hex, x_lab) {
  # Filter to display_name group so that we can order by group-wise median,
  #  while ensuring that 'Other' is still last
  df <- df %>%
  mutate(display_var = forcats::fct_reorder({{display_name}}, tumor_fraction),
...
# Cancer group
plot_tumor_violins(tumor_palette_df, cancer_group_display, cancer_group_hex, "Cancer group")

# Broad histology
plot_tumor_violins(tumor_palette_df, broad_histology_display, broad_histology_hex, "Broad histology")
```
