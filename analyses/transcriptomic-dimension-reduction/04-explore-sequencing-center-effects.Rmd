...
```{r}
# Wrap `cancer_group_display` for legend visibility as part of this plot:
# first, wrap palette labels so they apply!
#names(pal_cg) <- stringr::str_wrap(names(pal_cg), 20)

cancer_group_umap <- stranded_df %>%
  # wrap plot df labels
#  dplyr::mutate(cancer_group_display = stringr::str_wrap(cancer_group_display, 20)) %>%
  # remove NAs so theyre not in legend
