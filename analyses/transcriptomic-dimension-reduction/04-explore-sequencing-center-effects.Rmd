...
First we'll set up with the other data frame, but we don't set the palette since there is no pre-defined palette here.
```{r}
# only keep stranded for the rest and set order for `seq_center` as group size
...
# Legend does not render for cancer group colors properly, so create a hack legend first - 
legend_plot <- ggplot(stranded_df_subset) + 
  aes(x = UMAP1, y = UMAP2, color = cancer_group) + 
  geom_point() + 
  theme_classic() +
  theme(legend.position = "bottom") + 
  # make it 4 rows to fit in the final plot frame
  guides(color = guide_legend(nrow=4))
legend_panel <- cowplot::get_legend(legend_plot)


# plot itself:
umap_panel <-  ggplot(stranded_df_subset) + 
  aes(x = UMAP1, y = UMAP2, 
      shape = seq_center, 
      fill = cancer_group, 
      color = seq_center, 
...
# combine plot and color legend:
cowplot::plot_grid(umap_panel, legend_panel, 
