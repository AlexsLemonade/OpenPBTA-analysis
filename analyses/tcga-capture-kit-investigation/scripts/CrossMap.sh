...
for i in `ls ../../scratch/*.targetIntervals.Crossmapped_to_Gh38.bed`; do out=$(echo $i | rev| cut -d "/" -f1 |rev| sed 's/^/results\//g'); bedtools sort -i $i | bedtools merge -i - > $out; done
