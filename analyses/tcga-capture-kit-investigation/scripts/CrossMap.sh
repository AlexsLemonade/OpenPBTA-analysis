...
for i in `ls ../../scratch/*.targetIntervals.bed`; do out=$(echo $i | sed 's/.bed/.Gh38.bed/g'); CrossMap.py bed ../../scratch/hg19ToHg38.over.chain.gz $i $out; done

## This command sorts and merges all BED files in case there  are any overlaps in the BED files
for i in `ls ../../scratch/*.targetIntervals.Gh38.bed`; do out=$(echo $i | rev| cut -d "/" -f1 |rev| sed 's/^/results\//g'); bedtools sort -i $i | bedtools merge -i - > $out; done
