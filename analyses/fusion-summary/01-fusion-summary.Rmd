...
lgatFuses_df <- lgatFuses_df %>% 
  dplyr::mutate(Gene1A_anno = case_when(Gene1A %in% kinases ~ "Kinase"), 
                Gene1B_anno = case_when(Gene1B %in% kinases ~ "Kinase")) %>%
   replace_na(list(Gene1A_anno = "Non-Kinase", Gene1B_anno = "Non-Kinase"))
...
# Then, select the in-frame reciprocals which have the kinase domain in tact.
five_prime_reciprocals <- five_prime_domain_lost %>% 
...
subsetFuseLGAT <- bind_rows(nonkinaseLGAT,
                            three_prime_kinase_inframe[,c("Sample", "FusionName")], 
