...
prepareOutput <- function(fuseDF, bioid) {
  fuseDF %>% 
    ...
    dplyr::bind_rows(data.frame(Sample = setdiff(bioid, fuseDF$Sample))) %>%
    replace(is.na(.), 0)
}
...
Write the table with fusion counts to file for both tumor types.
Any sample in either caller should be represented in the final file.
