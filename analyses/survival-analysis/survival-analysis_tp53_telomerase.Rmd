...
```
Interaction terms are not significant, except `tp53_score:telomerase_score` at $P=`r fitted$p.value[fitted$term == "tp53_score:telomerase_score"]`$, which does not change much from individual term p-values, suggesting an additive model is more appropriate.
...
base_filename <- "cox_per_cg_terms_tp53_telomerase
_resect.RDS" # all file names here will follow this convention
