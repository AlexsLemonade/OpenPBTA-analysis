...
+ `meta_indep_hgg` which contains a new column `hgg_group` that contains levels (in order) `non-HGG` and `HGG` indicating whether it's an HGG sample
...
> non-HGG is the reference level.
...
fitted <- fit_save_model(meta_indep_hgg,    
...
```
Interaction terms are generally not significant, except `tp53_score:telomerase_score` which is only marginally significant at $P=`r fitted$p.value[fitted$term == "tp53_score:telomerase_score"]`$. This result suggests an additive model is more appropriate.
...
ONLY the EPN model shows significant effects for both `tp53_score` and `telomerase_score`, although CIs are fairly wide. 
