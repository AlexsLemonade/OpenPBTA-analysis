...
- Within each cancer group, TP53 classifier score and EXTEND score
...
### Assign categorical variable for non-HGAT or HGAT and select cancer groups
...
for(ind_var in c("tp53_score", "telomerase_score", "hgg_group", "cancer_group")){
...
## Log Rank analysis 
### Generate output for categorical data (cancer group only, here) 
```{r}
...
Initially, I thought we could assess `tp53_score*telomerase_score*cancer_group` here, but the removal of LGG and Ganglioglioma patients resulted in removal of 308 patients, and hence affects telomerase and tp53 predictions. 
...
Background on hazard ratios and output:
...
### Here, we ask how tp53 and telomerase affect survival within each cancer group
```{r}
## define multi-variates that we are using for analyzing survival
