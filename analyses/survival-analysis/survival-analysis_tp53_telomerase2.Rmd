...
```{r cox_function2}
fit_save_cox2 <- function(df, ind_variable, results_dir) {
  tryCatch({
  # Recode OS_Status
  df <- df %>%
    mutate(OS_status = ifelse(OS_status == "LIVING", 0, 1)) %>%
    # for some reason this function does not remove NAs, so we have to do it
    filter(!is.na(ind_variable))
  
  # Fit the model
  fit <- survivalAnalysis::analyse_multivariate(df,
                                                vars(time = OS_years, status = OS_status), covariates = ind_variable)

  # Save model
  
readr::write_tsv(fit$summaryAsFrame, 
                 file.path(
                   results_dir, 
                   paste0("survivalAnalysis_cox_reg_results_per", ind_variable, ".tsv")
                   )
                 ) 
...
```{r cox_tp53}
fit_save_cox2(meta_indep, vars(tp53_score), results_dir)
```
#### Telomerase score

```{r cox_telo}
fit_save_cox2(meta_indep, vars(telomerase_score), results_dir)
```
...
```{r cox_hgg}
fit_save_cox2(meta_indep_hgg, vars(hgg_group), results_dir)
```
