...
# Source functions used to build models
source(file.path(util_dir,"survival_models.R"))
source(file.path(util_dir,"survival_multivariate.R"))
source(file.path(util_dir,"survival_multivariate2.R"))
```
...
```{r cox_cancer_group}
fit_save_cox(meta_indep_cancer_groups, "cancer_group", results_dir)
```
...
Initially, it was planned to assess `tp53_score*telomerase_score*cancer_group` here, but the removal of LGG, Ganglioglioma, and Craniopharyngioma patients resulted in removal of 343 patients, and hence affects telomerase and tp53 predictions. Therefore this model will not be explored.
...
meta_indep_hgg <- meta_indep_hgg %>%
  mutate(OS_status = ifelse(OS_status == "LIVING", 0, 1))

result <- analyse_multivariate(meta_indep_hgg,
...
for (cg in cancer_groups_of_interest_df$cancer_group) {
  ...
  filename <- paste0("SurvivalAnalysis_cox_reg_results_per_tp53_score-by-telomerase_score_",
