...
kap_fit <- survival::survfit(
  survival::Surv(metadata$OS_days, metadata$OS_status_num) ~ pred,
  data = metadata
)
...
cowplot::plot_grid(surv_plot[[1]], surv_plot[[2]], nrow = 2, rel_heights = c(2.5, 1))
```
...
```{r Filter out unavailable genders}
# First let's filter out data that does not have pred available
metadata_filt <- metadata %>%
  dplyr::filter(pred != "Not Available")
```
...
logrank_fit <- survival::survdiff(
  survival::Surv(metadata_filt$OS_days, metadata_filt$OS_status_num) ~ pred,
  data = metadata_filt
)
```
