...
# check whether any RNA pts not in DNA subset
...
class(metadata_ind_dna$OS_status)
...
class(metadata$molecular_subtype)
...
```{r Kaplan Meier}
# this isn't working - error below `kap_fit`:
# Error: Column `OS_status` must be length 13 (the group size) or one, not 66


#kap_fit <- survival_analysis(metadata_ind_dna,
#  ind_var = metadata_ind_dna$molecular_subtype,
#  test = "kap.meier",
...
# try formula without function
meta <- metadata_ind_dna %>%
    dplyr::mutate(OS_status = as.numeric(ifelse(OS_status == "LIVING", 1,
                                    ifelse(OS_status == "DECEASED", 2, NA))))
  
kap_fit <- survfit(Surv(OS_years, OS_status) ~ molecular_subtype, data = meta)
kap_fit
plot_fit <- surv_summary(kap_fit)
```
...
surv_plot <- survminer::ggsurvplot(fit = plot_fit,
  data = meta,
  pval = TRUE,
  risk.table = TRUE,
  xlim = c(0, max(meta$OS_years)+1),
  break.time.by = 2,
...
cowplot::save_plot(filename = kap_meier_plot_file, plot = surv_plot, base_height = 4,  base_width = 8)
# Let's save the model for CI and median survial as well
readr::write_tsv(plot_fit, kap_meier_model_file)
```
...
Manual check - why am I not seeing H3 K28 subtypes?
...
fit <- coxph(Surv(OS_years, OS_status) ~ molecular_subtype, metadata_recode)
fit 
ggforest(fit)
