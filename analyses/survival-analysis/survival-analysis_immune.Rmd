...
histology_df <- histology_df %>%
  mutate(lgg_group = ifelse(short_histology == "LGAT",
                             lgg_levels[2], # second is HGG 
                             lgg_levels[1]), # first is non-HGG
          lgg_group = factor(lgg_group, levels = lgg_levels)) # ensures non-HGG is the reference 
...
quantiseq_terms <- "B_cell + Macrophage_M1 + Macrophage_M2 + Monocyte + Myeloid_dendritic_cell + Neutrophil + NK_cell + T_cell_CD4_non_regulatory + T_cell_CD8 + T_cell_regulatory_Tregs + extent_of_tumor_resection" 
