...
For the molecular signatures survival analysis, we only want to consider cancer groups that contain at least THREE DECEASED in each molecular subtype on a per-cancer group basis. We find those groups here - **there are no such groups, so we do not consider molecular subtype here.**

```{r cancer_groups_subtype}
# how many suitable molecular subtypes are there per cancer group?
suitable_subtypes_n <- combined_df_stranded %>%
  count(cancer_group, molecular_subtype, OS_status) %>%
  drop_na() %>%
  spread(OS_status, n,
         # When there are no events, record the event count as 0 instead of the default NA
         fill = 0) %>%
  filter(DECEASED >= 3) %>%
  count(cancer_group, name = "suitable_count")
  
# Does that match the total number of molecular subtypes for that cancer group, AND there are multiple subtypes?
# If so, we can proceed to model it.
suitable_cancer_groups_subtypes <- combined_df_stranded %>%
  select(cancer_group, molecular_subtype) %>%
  distinct() %>%
  # how many molecular subtypes does cancer group have?
  count(cancer_group) %>%
  # Are these cancer groups suitable? 
  filter(cancer_group %in% suitable_subtypes_n$cancer_group) %>%
  inner_join(suitable_subtypes_n) %>%
  filter(n == suitable_count, 
         n > 1) %>% 
  pull(cancer_group)

suitable_cancer_groups_subtypes
```
