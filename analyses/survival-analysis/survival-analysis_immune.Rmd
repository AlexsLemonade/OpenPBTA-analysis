...
```
## Re-analyze with stranded only: PDL-1 expression minus samples with total resection

> There is no signficant effect of PDL-1 expression. Although this P-value could be viewed as "approaching signficance," the large analysis sample size (N=719), this P-value is actually rather large.


```{r cox_pdl1 not total resection}
# How many samples had a full tumor resection? (best predictor of survival)
as.data.frame(table(combined_df_stranded$extent_of_tumor_resection))

# Remove these - now only 273 samples
resect_removed <- combined_df_stranded[!grepl("Gross/Near total resection", combined_df_stranded$extent_of_tumor_resection),]
nrow(resect_removed)

fit_save_model(resect_removed, 
               "CD274",
               file.path(results_dir, "cox_terms_pdl1_stranded_not_resected.RDS"))

# how many LGG? 66, remove
as.data.frame(table(resect_removed$cancer_group))

lgg_removed <- resect_removed %>%
  filter(cancer_group != "Low-grade glioma astrocytoma")

fit_save_model(lgg_removed, 
               "CD274",
               file.path(results_dir, "cox_terms_pdl1_stranded_lgg_rm.RDS"))
...
## Analysis: quantiseq cell type fractions across entire cohort, minus total resection
...
# saving so P-values can be programmatically grabbed
fit <- fit_save_model(lgg_removed,
                         quantiseq_terms,
                         file.path(results_dir, "cox_terms_quantiseq_not_lgg.RDS"),
                         model_type = "multivariate")
fit
```
