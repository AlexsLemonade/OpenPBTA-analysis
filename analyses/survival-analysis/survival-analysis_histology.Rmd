...
tel_polya_file <- file.path(analysis_dir,
                         "telomerase-activity-prediction/results/TelomeraseScores_PTBAPolya_FPKM.txt")
```
...
ind_var <- "tp53_score+telomerase_score+hgg_group+tp53_score:telomerase_score+hgg_group:tp53_score+hgg_group:telomerase_score"
...
# define multi-variates that we are using for analyzing survival
ind_var <- "tp53_score+telomerase_score+tp53_score:telomerase_score"
# define model
model <- paste0("survival::Surv(time = OS_years, event = OS_status_fixed) ~ ", ind_var)
