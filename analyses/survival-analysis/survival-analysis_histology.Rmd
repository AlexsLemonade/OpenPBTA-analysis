...
histologies <- readr::read_tsv(metadata_file, guess_max = 10000) %>%
  ...
  dplyr::select(sample_id, broad_histology_display, cancer_group, OS_status, OS_days, PFS_days) %>%
  distinct()
...
for(ind_var in c("tp53_score", "telomerase_score", "broad_histology_display", "hgg_group")){
...
```{r}
ind_var_list <- c("hgg_group", "broad_histology_display")
...
```{r}
for(ind_var in c("tp53_score+telomerase_score+hgg_group", "tp53_score+telomerase_score+broad_histology_display")){
  model <- paste0("survival::Surv(time = OS_years, event = OS_status_fixed) ~ ", ind_var)
...
## Bivariate density plot of tp53 score and telomerase activity score
```{r}
