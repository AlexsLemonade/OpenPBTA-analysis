...
histologies <- readr::read_tsv(metadata_file, guess_max = 10000) %>%
  ...
  dplyr::select(Kids_First_Participant_ID, sample_id, cancer_group, OS_status, OS_days, PFS_days, experimental_strategy, RNA_library) %>%
  distinct() %>%
  dplyr::arrange(sample_id)

# get independent participants with primary tumor, RNA-Seq that is stranded
indep_participants <- histologies %>%
  dplyr::filter(experimental_strategy == "RNA-Seq" & RNA_library == "stranded") %>%
  pull(Kids_First_Participant_ID) %>% 
  unique()

# filter histologies to remove unnecessary columns 
histologies <- histologies %>% 
  dplyr::select(-c(experimental_strategy, RNA_library)) %>%
  distinct()
...
meta_indep <- tp53_scores %>%
  ...
  dplyr::arrange(Kids_First_Biospecimen_ID_RNA) %>%
  dplyr::filter(Kids_First_Participant_ID %in% indep_participants) %>%
