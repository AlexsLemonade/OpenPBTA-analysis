...
survival_analysis <- function(metadata, 
  ...
  if (!all(found_cols)) {
    stop(cat("The following column names specified for overall survival information: \n", 
  ...
  metadata <- metadata %>% 
    dplyr::mutate(os_status = factor(os_status, levels = c("LIVING", "DECEASED")), 
  ...
  if (!is.null(ind_data)) {
    ...
    # List the columns we need
    found_cols <- c(ind_data_sample_col %in% colnames(ind_data), 
                   ind_var %in% colnames(ind_data),
                   metadata_sample_col %in% colnames(metadata))
    
    # If not all the columns are found, stop
    if (!all(found_cols)) {
      stop(cat("The following column names specified: \n", 
               paste(needed_cols[which(found_cols)], collapse = "\n"), 
    ...
    metadata <- dplyr::inner_join(metadata, ind_data,
      by = c(metadata_sample_col = ind_data_sample_col)
      )
    ...
    # If not all the columns are found, stop
    if (!all(found_cols)) {
      stop(cat("The following column names specified for the independent variable(s): \n", 
  ...
  ind_var_df <- metadata %>%
    dplyr::select(metadata_sample_col, {{os_days_col}}, "os_status", ind_var)
