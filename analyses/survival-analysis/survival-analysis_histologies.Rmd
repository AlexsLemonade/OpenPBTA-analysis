...
# meta_indepine metadata, remove cell lines and samples without RNA-Seq since both classifiers' results use RNA-Seq
...
meta_indep <- meta_indep %>%
  filter(Kids_First_Biospecimen_ID_RNA %in% indep_rna$Kids_First_Biospecimen_ID)
...
```{r}
phenotypes <- c("tp53_strict", "tel_strict", "pheno_0.5", "pheno_strict", "pheno_extremes")
```
...
kap_fit <- survival_analysis(meta_hgat,
                             ind_var = each,
                             test = "kap.meier",
