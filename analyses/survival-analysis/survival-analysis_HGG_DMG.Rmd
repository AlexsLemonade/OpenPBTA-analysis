...
count_patient_subtype <- metadata %>%
  dplyr::select(Kids_First_Participant_ID, molecular_subtype) %>%
  unique() %>%
  group_by(Kids_First_Participant_ID ) %>%
  dplyr::summarise(count = n(), subtype = toString(molecular_subtype)) %>%
  dplyr::filter(count > 1)

```

#### Using independent_primary if multiple subtypes per patient are found 
This way we use only primary samples only and maybe will help filter cases where multiple subtypes are present per patient ID
...
if (nrow(count_patient_subtype) > 0) {
  warning(glue::glue_collapse(
    count_patient_subtype$Kids_First_Participant_ID,
    sep = ",")
          , "have are patients with multiple subtypes.")
  warning("Using independent primary list to get individual samples")
