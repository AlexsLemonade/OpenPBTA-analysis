...
target_column_list <- strsplit(argsL$target_columns, ",")
df_list <- list()

# Create specified output directory if it does not yet exist
plot_directory <- file.path(argsL$results_dir, "plots")
...
# here, we collect the predictive accuracy values from each confusion matrix file in the results directory  
  results_directory <- file.path(argsL$results_dir, t)
  files_to_process <- list.files(path = results_directory, pattern = "_confusion_matrix.RDS")
  first_pos <- sapply(files_to_process, function(x) regexpr(argsL$seed, x))
  last_pos <- sapply(files_to_process, function(x) regexpr("confusion_matrix", x))
  transcript_tail_percents <- substr(files_to_process, 1, last_pos-2)
  
  files_to_process_names <- file.path(argsL$results_dir, t, files_to_process)
...
#store predictive accuracies in a data frame with ID=unique portion of the confusion matrix file name  
  predictive_accuracies <- data.frame(ID=transcript_tail_percents, 
...
# # here, we collect the # training transcripts values from each model object in the models directory  
  results_directory <- file.path(argsL$model_dir)
  files_to_process <- list.files(path = results_directory, pattern = "_model_object.RDS")
  first_pos <- sapply(files_to_process, function(x) regexpr(argsL$seed, x))
  last_pos <- sapply(files_to_process, function(x) regexpr("model_object", x))
  transcript_tail_percents <- substr(files_to_process, 1, last_pos-2)
  
  files_to_process_names <- file.path(argsL$model_dir, files_to_process)
...
#store # training transcripts values in a data frame with ID=uniqure portion of the model object file name 
  number_transcripts <- data.frame(ID=transcript_tail_percents, number_training_transcripts=number_transcripts)
...
#here, we collect the number of non-zero features from each model coefficients file in the models directory  
  ...
  first_pos <- sapply(files_to_process, function(x) regexpr(argsL$seed, x))
  last_pos <- sapply(files_to_process, function(x) regexpr("model_coefs", x))
  transcript_tail_percents <- substr(files_to_process, 1, last_pos-2)
  
  files_to_process_names <- file.path(argsL$model_dir, files_to_process)
  number_non_zero <- sapply(files_to_process_names, function(x) 
    nrow(read.delim(x, header=TRUE, sep="\t", stringsAsFactors = FALSE)))

#store # non-zero features values in a data frame with ID=unique portion of the model coeffs file name  
  number_non_zero <- data.frame(ID=transcript_tail_percents, number_features=number_non_zero)
...
#store this data frome in the list of data frames by targetColumn
  df_list[[t]] <- final_df
...
The model generates a probability that a given sample is Female. Female/Male calls are made using a 50% threshold. Predictive accuracy varies by the number of training transcripts used.  The plot below shows model probability values, generated by the most accurate set of training transcripts, for all 258 samples in the test set.  The test set points are split on the x-axis by reported_gender (ground truth) values. Erroneous calls are displayed in red.  


```{r echo=FALSE}

#for each value in targetColumns argument, find the index of the row corresponding to the maximum predictive accuracy
#if more than one match, take the largest index value
max_accuracy_index <- max(which(df_list[[target_column_list[[1]][1]]]$predictive_accuracy==
                                  max(df_list[[target_column_list[[1]][1]]]$predictive_accuracy)))

#construct the name of the matching prediction details file  
start_name <- df_list[[target_column_list[[1]][1]]]$cm_filename[max_accuracy_index]
final_name <- paste(substr(start_name, 1, regexpr("_confusion", start_name)-1), "_prediction_details.tsv", sep="")
  
#read the matching prediction details file  
df_name <- file.path(final_name)
...
plot <- ggplot(plot_df, aes(x=obs, y=Female)) + 
  ...
  "red", "black"))) + labs(title=paste("Strength of Calls at Maximum Accuracy"), 
...
suppressMessages(ggsave(plot=plot, filename=file.path(plot_directory,
                                       paste("Strength_of_Calls_for_at_Maximum_Accuracy", ".png", sep=""))))
...
Model accuracy was tested against germline_sex_estimate values in addition to reported_gender values.  Confusion matrices for models using the most accurate set of training transcripts appear below.
...
for (t in target_column_list[[1]]) {
  ...
  summary_file_name <- substr(df_list[[t]]$cm_filename[max_accuracy_index], 1,
                              regexpr("_confusion_matrix", df_list[[t]]$cm_filename[max_accuracy_index]))
  summary_file_name <- paste(summary_file_name, "two_class_summary.RDS", sep="")
...
Training transcripts are filtered by median absolute deviation across all samples.  The pipeline accepts an array of user-supplied training set sizes, and a model, using each training set size, is built, evaluated and saved. The plots below show predictive accuracies by number of training transcripts using reported_gender and germline_sex_estimate as ground truths.
...
The number of non-zero features in each model varied considerably.  The elastic net alpha parameter was optimized using the default sequence of 11 values more closely spaced around alpha = 0.  
