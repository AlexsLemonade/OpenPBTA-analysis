...
transposed_gistic <- transposed_gistic %>%
  ...
  dplyr::mutate(copy_number = as.numeric(V1)) %>%
...
final_gistic <-  transposed_gistic %>%
  dplyr::mutate(GISTIC_SMARCB1_focal_status = dplyr::case_when(
    ...
    copy_number < tumor_ploidy ~ "loss",
    # If copy number is higher than ploidy, mark as a gain
    copy_number > tumor_ploidy ~ "gain",
    copy_number == tumor_ploidy ~ "neutral"
  ))
```
...
comparison_plot <-
  ...
  ggplot2::geom_bar(ggplot2::aes(fill = GISTIC_SMARCB1_focal_status))
