...
# Install `ComplexHeatmap` package for creating Heatmap annotation objects
...
## Prepare files for plotting of initial heatmap
...
# Write initial annotation object to file
readr::write_rds(initial_ha_atrt,
...
# tyr_genes_collapsed <- paste(tyr_genes, collapse = "|")
# shh_genes_collapsed <- paste(shh_genes, collapse = "|")
# myc_genes_collapsed <- paste(myc_genes, collapse = "|")
...
cn_metadata <- cn_df %>%
  ...
  dplyr::select(gene_symbol,
                ...
                status) %>%
  dplyr::filter(
    sample_id %in% atrt_expression_df$sample_id,
    gene_symbol %in% tyr_genes |
      gene_symbol %in% shh_genes |
      gene_symbol %in% myc_genes |
      gene_symbol %in% c("SMARCB1", "SMARCA4")
  ) %>%
  dplyr::mutate(
...
final_df <- atrt_expression_cn_tmb_df %>%
  ...
  dplyr::mutate(
    Kids_First_Biospecimen_ID = paste(sort(
      unique(Kids_First_Biospecimen_ID.x, Kids_First_Biospecimen_ID.y)
    ), collapse = ", "),
    Kids_First_Participant_ID = paste(sort(
      unique(Kids_First_Participant_ID.x, Kids_First_Participant_ID.y)
    ), collapse = ", ")
  ) %>%
  dplyr::select(
    -c(
...
# Prepare files for plotting of final heatmap
...
final_ha_atrt <- HeatmapAnnotation(df = annotation_mat)

# Write final annotation object to file
readr::write_rds(final_ha_atrt,
