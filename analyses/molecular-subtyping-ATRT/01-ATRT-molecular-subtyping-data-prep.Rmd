...
missing_cn_samples <- metadata %>%
  ...
  dplyr::mutate(SMARCB1_focal_status = "neutral",
                SMARCA4_focal_status = "neutral")

genes_cn_df <- genes_cn_df %>%
  dplyr::bind_rows(missing_cn_samples)

rm(missing_cn_samples)
...
# # Replace `NA` with `neutral` for focal status variables (for WGS samples)
# final_df <- final_df %>%
#   dplyr::group_by(sample_id) %>%
#   dplyr::mutate(
#     SMARCB1_focal_status = paste(sort(unique(dplyr::case_when(
#       (is.na(as.character(SMARCB1_focal_status)) | SMARCB1_focal_status == "NA") &
#         !(
#           Kids_First_Biospecimen_ID %in% subset_metadata$Kids_First_Biospecimen_ID
#         ) ~ "neutral",
#       (is.na(as.character(SMARCB1_focal_status)) | SMARCB1_focal_status == "NA") &
#         (
#           Kids_First_Biospecimen_ID %in% subset_metadata$Kids_First_Biospecimen_ID
#         ) ~ "NA",
#       TRUE ~ as.character(SMARCB1_focal_status)
#     ))), collapse = ", "),
#     SMARCA4_focal_status = paste(sort(unique(dplyr::case_when(
#       (is.na(as.character(SMARCA4_focal_status)) | SMARCA4_focal_status == "NA") &
#         !(
#           Kids_First_Biospecimen_ID %in% subset_metadata$Kids_First_Biospecimen_ID
#         ) ~ "neutral",
#       (is.na(as.character(SMARCA4_focal_status)) | SMARCA4_focal_status == "NA") &
...
#           Kids_First_Biospecimen_ID %in% subset_metadata$Kids_First_Biospecimen_ID
#         ) ~ "NA",
#       TRUE ~ as.character(SMARCA4_focal_status)
#     ))), collapse = ", ")
#   ) %>%
#   dplyr::distinct()
