...
First, add comparables as their own column to the metadata table for later use.
```{r add to metadata}
...
To start, we will reproduce an naive dimensionality reduction analysis to show the problem of selection method that we are facing. Sinc UMAP seems to work well overall, that is what we will focus on first.
...
So even with more dimensions, we are not suprised to see that the PolyA samples are still clustering together.

## Removing PolyA depleted genes
...
One more principaled effort would be to see if we can apply ComBat and get anything reasonable.
...
It is possible that some specific analyses may be salvageable with ComBat or a similar method, but the overall reccomendation is likely to be to do most of the expression analysis treating the stranded and polyA selected libraries separately, and not attempt to draw conclusions about comparsions between these two sets.
