...
ews_subtype <- ews_rnaseq %>% 
  ...
  dplyr::mutate(molecular_subtype=
                  # if sample_id in has_ews_fusion_sampleid
                  case_when(sample_id %in% has_ews_fusion_sampleid ~ "EWS",
                            # if sample_id not in has_ews_fusion_sampleid then 
                            # have molecular_subtype == "To be classified"
                            TRUE ~ "EWS, To be classified")
  )
```
