...
sigfit_mean %>%
  ...
  tibble::as_tibble()  %>%
  dplyr::mutate(type = "sigfit") -> sigfit_exposures


# deconstructSigs means exposures into a tibble
lapply(decon_rds, "[[", "weights") -> decon_mean
do.call(rbind, decon_mean) %>%
  tibble::rownames_to_column("sample_id") %>%
  tibble::as_tibble() %>%
  dplyr::mutate(type = "decon") -> decon_exposures
```
...
dplyr::full_join(sigfit_exposures, decon_exposures) %>%
  tidyr::gather(-sample_id, -type, key = "signature", value = "weight") %>%
  tidyr::spread(key = "type", value = "weight") -> data_compare_types
...
lower_bound %>%
  ...
  dplyr::mutate(sigfit_lb_OK = sigfit_lb >= 0.01) -> data_compare_types
...
data_compare_types %>%
  dplyr::mutate(sigfit = dplyr::if_else(sigfit_lb_OK, sigfit, 0)) -> data_compare_types_filter
