...
if (file.exists(de_novo_file))
{
  ...
  # This takes quite a few minutes to parse.
  ...
  readr::write_rds(de_novo_signatures, de_novo_file)
}
```
...
tibble::tibble(
  ...
  tidyr::unnest(cols=cosmic_sig_index) %>%
...
**Now specifically for the results whose k was selected by cosine similarity goodness-of-fit**, among *k in [2:5]* for each the given random seed, using 3000 iterations:
...
The above table shows matched COSMIC SBS signatures under a given random seed and inference model. Highlighted rows were selected using cosine similarity goodness-of-fit 
...
```{r count_sigs}
unnested_sigs %>%
  dplyr::count(cosmic_sig) %>%
  dplyr::arrange(-n) %>%
  dplyr::slice(1:10) %>%
  dplyr::rename(Signature = cosmic_sig, count = n)
```
...
+ **[SBS26](https://cancer.sanger.ac.uk/signatures/sbs/sbs26/)** "is one of seven mutational signatures associated with defective DNA mismatch repair and microsatellite instability (MSI) and is often found in the same samples as other MSI associated signatures: SBS6, SBS14, SBS15, SBS20, SBS21, and SBS44."
...
**The three bolded** signatures are commonly associated with one another due to putative shared mechanisms of "defective DNA mismatch repair and microsatellite instability (MSI)."