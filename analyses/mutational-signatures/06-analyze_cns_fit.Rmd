...
# Read in exposures from CNS signature fitting
raw_exposures <- readr::read_rds(fitted_file)
exposures_mean <- raw_exposures$mean
# Set exposures to 0, if lower confidence interval hits 0.01
# https://github.com/AlexsLemonade/OpenPBTA-analysis/pull/678/files#diff-83a49ed371610e147e8d2a5560157f05a013aaa4f2d67c81b5164da7e2687140R436
# "sufficiently non-zero" threshold per vignette, too
exposures_mean[raw_exposures$lower_95 < 0.01] <- 0
...
# Sample mutation counts per WGS Mb
rowSums(sigs_input) %>%
  # throws a warning - ok for us since our package behavior is not going to change in Docker.
  tibble::as_tibble(rownames = "Kids_First_Biospecimen_ID") %>%
  dplyr::rename(mutation_count = "value") %>%
  ## Original comment said sigfit doesn't consider samples with <50, but I think we see some non-zero exposures that suggest otherwise
  ## also probably makes sense to include these in the denominator wrt to proportion of tumors with signature
...
# Write signature counts to file for downstream use
readr::write_tsv(signature_counts, file.path(analysis_path, "results", "fitted_cns_signature_counts.tsv"))
```
<br><br>
