...
anno <- tmb_coding %>%
  ...
  select(Patient, Kids_First_Biospecimen_ID, `Phase of Therapy`, Mutator) %>%
  as.data.frame()
...
ann_colors = list(
  ...
  `Phase of Therapy` = c(`Initial CNS Tumor` = "#709AE1", 
                       `Progressive` = "#38736D"))
# Make heatmap
heat_plot <- pheatmap(sigs[, c("1", "3", "8", "11", "18", "19", "MMR2", "N6")], 
         cluster_rows = F, 
         cluster_cols = F, 
         border_color = "white",
...
ggsave(heatmap_file, heat_plot, width = 5, height = 4)
