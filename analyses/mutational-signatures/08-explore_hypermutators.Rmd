...
# Read in tumor descriptor palette
...
anno <- tmb_coding %>%
  ...
  select(Patient, Kids_First_Biospecimen_ID, `Phase of Therapy`, `Mutation status`) %>%
  as.data.frame() 
...
`Mutation staatus` = c(Normal = "grey40",
              Hypermutant = "orange",
...
heat_plot <- pheatmap(sigs[, c("1", "3", "8", "11", "18", "19", "MMR2", "N6")], 
         ...
         cluster_cols = F, 
         border_color = "white",
         color=colorRampPalette(c("#f1f1f1", "#2166ac"))(50),
         gaps_row = c(2,3,5,6),
         annotation_row = anno,
