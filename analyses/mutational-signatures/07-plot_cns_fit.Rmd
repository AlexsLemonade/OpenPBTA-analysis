...
### Exposure barplot across broad histologies

To color by signature group, we use the `simpsons` palette from `ggsci`, as [described](https://github.com/AlexsLemonade/OpenPBTA-analysis/tree/master/figures#overall-figure-theme). _However_, because some pairs of colors are challening to distinguish in the first 8 colors of the palette, we take the first 6 and the last 2 simpsons colors. 
...
exposure_barplot <- df_signature_mutations_per_mb %>%
  dplyr::group_by(broad_histology_display, signature) %>%
  dplyr::summarize(median_exposure = median(exposure)) %>%
  dplyr::ungroup() %>%
  dplyr::inner_join(
    dplyr::select(palette_df, 
           broad_histology_display, 
           broad_histology_order)
  ) %>%
  dplyr::mutate(broad_histology_display = stringr::str_wrap(broad_histology_display, 25), 
                broad_histology_display = forcats::fct_reorder(broad_histology_display,
                                                             broad_histology_order), 
                signature = forcats::fct_rev(signature)) %>%
  dplyr::distinct() %>%
  ggplot2::ggplot() + 
...
ggplot2::ggsave(exposure_barplot_file, exposure_barplot, width = 8, height = 5)
```
