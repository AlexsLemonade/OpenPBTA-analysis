...
exposure_counts <- df_signature_mutations_per_mb %>%
  ...
  dplyr::ungroup() %>%
  dplyr::mutate(
    cancer_group_display2 = as.character(glue::glue("{cancer_group_display}\n(n={total})"))
  )



display2_hex <- palette_df %>%
  dplyr::inner_join(exposure_counts) %>%
  dplyr::select(cancer_group_hex, cancer_group_display2) %>%
  dplyr::distinct()


cancer_group_palette <- display2_hex$cancer_group_hex
names(cancer_group_palette) <- display2_hex$cancer_group_display
