...
exposures_sina_boxplot <- results_df %>%
  # Should we visualize Unknown here also?
...
ggplot2::ggsave(exposures_presence_barplot_file, exposures_proportions, width = 12, height = 5)
...
cancer_groups_of_interest <- c("High-grade glioma astrocytoma", "Medulloblastoma")
...
sig1_exposures <- results_df %>%
  ...
  # filter only to cancer groups of interest for this analysis
  dplyr::filter(cancer_group_display %in% cancer_groups_of_interest)
```
...
```{r sig1_jitter, fig.width = 8, fig.height = 4}
...
sig1_exposures %>%
  ...
  ggplot2::facet_wrap(~cancer_group_display,nrow = 1) +
  ggplot2::labs(
    x = "Tumor descriptor",
...
# We can also viz specifically HGG, H3 wildtype samples which have sample sizes that are able to be visualized
sig1_exposures %>%
  dplyr::filter(stringr::str_detect(molecular_subtype, "wildtype")) %>%
  ggplot2::ggplot() + 
  ggplot2::aes(x = tumor_descriptor, 
               y = sig1_proportion, 
               color = tumor_descriptor) + 
  ...
  # add in mean guiding point
  ggplot2::stat_summary(color = "black") + 
  ggplot2::facet_wrap(~molecular_subtype,nrow = 1) +
  ggplot2::labs(
    x = "Tumor descriptor",
    y = "Signature 1 Exposure"
  ) +
