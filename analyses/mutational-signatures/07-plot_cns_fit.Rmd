...
First, we read in data and prepare exposures and TMB information.
...
```{r boxplot, fig.width = 5, fig.height = 8}
...
ggplot2::ggsave(exposures_sina_IQR_file, exposures_sina_boxplot, width = 5, height = 8)
...
This plot shows the literal exposures for each samples.
We order plots by sample 1, and we include the unknown proportion so everything has the same height. 
In this case, "Unknown" specifically means mutations that were not assigned to the specific 8 CNS signatures examined, but are associated (if at all) with a different signature.
...
### Explore: MMR2 signature and mutation burden

Which samples have MMR ~ 1, and are these hypermutators? 
This analysis is discussed [here](https://github.com/AlexsLemonade/OpenPBTA-analysis/pull/1227#issuecomment-1064493652).

Note that this visualiation is strictly exploratory and not exported. 

We do **not** observe that samples with high MMR2 exposure have high mutational burdens. We do see that the majority of all such samples are subtyped as HGG, H3 wildtype.

```{r mmr2, fig.width = 10, fig.height = 3}
top_mmr2 <- results_df %>%
  dplyr::filter(signature == "MMR2") %>%
  dplyr::select(exposure, Kids_First_Biospecimen_ID, cancer_group_display) %>%
  dplyr::distinct() %>%
  dplyr::arrange(-exposure) %>%
...
# We see in this visualization that very few samples are >0.5 MMR2 exposure,
# and that the samples with extremely high mutation counts are NOT the same as those with high MMR2 exposure
top_mmr2 %>%
  ggplot2::ggplot() + 
  ggplot2::aes(x = exposure, y = tmb, fill = cancer_group_display) +
  ggplot2::geom_point(shape = 21, color ="black") + 
  ggsci::scale_fill_simpsons() +
  ggplot2::labs(x = "Exposure to MMR2") +
  ggplot2::facet_wrap(~tmb_type, scales = "free_y")

# What are the specific samples that are > MMR2 exposure 0.75? (Since so few samples are this high, we can view this more below ~1.)
top_mmr2 %>%
  dplyr::filter(exposure >= 0.75) %>%
  dplyr::select(-dplyr::contains("tmb")) %>%
  dplyr::distinct() %>%
  dplyr::inner_join(
    meta %>%
...
##### Explore: High TMB signatures

When exploring MMR2, we see that the four extremely mutated samples are _not_ dominantly exposed to MMR2. What are these samples, and what signatures are they exposed to?

All four samples are High-grade glioma astrocytoma with subtype	`HGG, H3 wildtype`, and two are tp53 loss while the other two are tp53 activated.
All of these samples have _no exposure_ to signatures 3, 8, or N6. Two of the samples (one tp53 loss and other tp53 activated) have no exposure to signature 19. 

```{r hypermut}
# Grab the IDs for the four samples
hypermut_ids <- tmb_data %>%
  dplyr::filter(tmb > 2) %>%
  dplyr::pull(Kids_First_Biospecimen_ID)
...
subtypes <- meta %>%
  dplyr::filter(Kids_First_Biospecimen_ID %in% hypermut_ids) %>%
  dplyr::select(Kids_First_Biospecimen_ID, molecular_subtype) 

# Table of merged information about these four samples:
results_df %>% 
  dplyr::filter(Kids_First_Biospecimen_ID %in% hypermut_ids) %>%
  dplyr::select(Kids_First_Biospecimen_ID, cancer_group_display, exposure, signature) %>%
  dplyr::inner_join(subtypes) %>%
