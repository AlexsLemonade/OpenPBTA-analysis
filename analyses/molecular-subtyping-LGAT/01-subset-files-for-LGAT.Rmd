...
consensusMutationSubset <- lgat_dna_df %>%
  ...
  dplyr::mutate(BRAF_V600E_mut = if_else(any(colnames(.) %in% snvOI$BRAF_V600E$gene) & 
                                     rowSums(dplyr::select(.,one_of(snvOI$BRAF_V600E$gene))) >=1,
                                     "Yes","No"),
                FGFR_mut = if_else(any(colnames(.) %in% snvOI$FGFR$gene) & 
                                     rowSums(dplyr::select(.,one_of(snvOI$FGFR$gene))) >=1
                                     ,"Yes","No"),
                IDH_mut  = if_else(any(colnames(.) %in% snvOI$IDH1$gene) & 
                                     rowSums(dplyr::select(.,one_of(snvOI$IDH1$gene))) >=1,
                                   "Yes","No"),
                H3.1_mut = if_else(any(colnames(.) %in% c("HIST1H3B","HIST1H3C")) & 
                                     rowSums(dplyr::select(.,one_of(c("HIST1H3B","HIST1H3C")))) >=1,
                                   "Yes","No"), 
                ...
                MAPK_mut  = if_else(any(colnames(.) %in% snvOI$MAPK$gene) & 
                                     rowSums(dplyr::select(.,one_of(snvOI$MAPK$gene))) >=1,
                                             "Yes","No"),
                RTK_mut  = if_else(any(colnames(.) %in% snvOI$RTK$gene) & 
                                     rowSums(dplyr::select(.,one_of(snvOI$RTK$gene))) >=1,
                                   "Yes","No"),
                NF1_mut = if_else(any(colnames(.) %in% snvOI$NF1$gene) & 
                                     rowSums(dplyr::select(.,one_of(snvOI$NF1$gene))) >=1,
                                  "Yes","No") ) %>%
