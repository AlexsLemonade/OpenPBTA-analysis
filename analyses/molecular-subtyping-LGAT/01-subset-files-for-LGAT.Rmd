...
snv_consensus_hotspot_maf_subset <- snv_consensus_hotspot_maf  %>%
  # find lgat samples
...
snv_consensus_hotspot_maf_subset
```
...
snv_consensus_hotspot_maf_subset <- snv_consensus_hotspot_maf_subset %>%
...
```{r}
all(lgat_dna_df$Kids_First_Biospecimen_ID %in% snv_consensus_hotspot_maf_subset$Tumor_Sample_Barcode)
```
...
recode_mutations <- function(.data, genes) {
  if_else(any(colnames(.data) %in% genes) & 
            rowSums(dplyr::select(.data, one_of(genes))) >= 1,
          "Yes", 
          "No"
          )
}

snv_consensus_hotspot_maf_subset <- lgat_dna_df %>%
  left_join(snv_consensus_hotspot_maf_subset,by=c("Kids_First_Biospecimen_ID"="Tumor_Sample_Barcode")) %>%
  reshape2::dcast(Kids_First_Biospecimen_ID + sample_id~ Hugo_Symbol,
                  fun.aggregate = length) %>%
  dplyr::mutate(BRAF_V600E_mut = recode_mutations(., genes = snvOI$BRAF_V600E$gene),
                FGFR_mut = recode_mutations(., genes = snvOI$FGFR$gene),
                IDH_mut  = recode_mutations(., genes = snvOI$IDH1$gene),
                H3.1_mut = recode_mutations(., genes =  c("HIST1H3B","HIST1H3C")), 
                # No HIST1H3C, HIST2H3C, or H3F3A mutations found
                H3.2_mut = recode_mutations(., genes =  snvOI$HIST2H3C$gene) , 
                H3.3_mut = recode_mutations(., genes = c("H3F3A","H3F3B")) ,
                MAPK_mut  = recode_mutations(., genes = snvOI$MAPK$gene) ,
                RTK_mut  = recode_mutations(., genes = snvOI$RTK$gene),
                NF1_mut = recode_mutations(., genes = snvOI$NF1$gene) 
                ) %>%
...
write_tsv(snv_consensus_hotspot_maf_subset,file.path(subset_dir, "LGAT_snv_subset.tsv"))
```
