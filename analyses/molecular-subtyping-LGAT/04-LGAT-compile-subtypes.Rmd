...
all_lgat_matched <- full_join(lgat_wgs, lgat_rna,
...
nf1_snv <- lgat_snv %>%
  filter(NF1_mut=="Yes") %>%
  mutate(molecular_subtype = "LGG, NF1") %>%
...
nf1_germline <- lgat_manifest %>% 
  ...
  filter(grepl("NF", cancer_predispositions)) %>%
  mutate(molecular_subtype = "LGG, NF1") %>%
  select("Kids_First_Biospecimen_ID","molecular_subtype") 
...
```{r}
kiaabraf_biospecimen <- lgat_fusion %>%
  filter(KIAA_BRAF_fus=="Yes") %>%
  mutate(molecular_subtype = "LGG, KIAA1549-BRAF") %>%
  select("Kids_First_Biospecimen_ID","molecular_subtype") 
...
This subtype contains BRAF V600E or V599 SNV or non-canonical BRAF alterations such as p.V600ins or p.D594N
```{r}

brafV600_biospecimen <- lgat_snv %>%
  filter(BRAF_V600E_mut=="Yes") %>%
  mutate(molecular_subtype = "LGG, BRAF V600E") %>%
  select("Kids_First_Biospecimen_ID","molecular_subtype") 
...
otherMAPK_fus_biospecimen <- lgat_fus %>%
  filter(MAPK_fus=="Yes") %>%
  mutate(molecular_subtype = "LGG, other MAPK") %>%
  select("Kids_First_Biospecimen_ID","molecular_subtype")
...
rtk_fus_biospecimen <- lgat_fus %>%
  filter(RTK_fus=="Yes") %>%
  mutate(molecular_subtype = "LGG, RTK") %>%
  select("Kids_First_Biospecimen_ID","molecular_subtype") 
...
OR harbors FGFR1-TACC1 fusion
OR harbors FGFR1 or FGFR2 fusions
```{r}
fgfr_fus_biospecimen <- lgat_fus %>%
  filter(FGFR_fus=="Yes") %>%
  mutate(molecular_subtype = "LGG, FGFR") %>%
  select("Kids_First_Biospecimen_ID","molecular_subtype") 
...
idh_biospecimen <- lgat_snv %>%
  filter(IDH_mut=="Yes") %>%
  mutate(molecular_subtype = "LGG, IDH") %>%
  select("Kids_First_Biospecimen_ID","molecular_subtype") 
...
h3_biospecimen <- lgat_snv %>%
  ...
  mutate(molecular_subtype = "LGG, H3") %>%
  select("Kids_First_Biospecimen_ID","molecular_subtype") 
...
myb_biospecimen <- lgat_fus %>%
  filter(MYB_fus=="Yes") %>%
  mutate(molecular_subtype = "LGG, MYB/MYBL1") %>%
  select("Kids_First_Biospecimen_ID","molecular_subtype")
```
...
cdkn_biospecimen <- lgat_cnv %>%
  ...
  mutate(molecular_subtype = "LGG, CDKN2A/B") %>%
  select("Kids_First_Biospecimen_ID","molecular_subtype") 
```
...
all_subtype <- bind_rows(all_rna_subtype,
  ...
  mutate( molecular_subtype = case_when(
    # multiple subtypes are summarised 
    # with a redudant ",LGG," from instances 
    # like molecular_subtype ==LGG, BRAF V600E, LGG, CDKN2A/B
    grepl(", LGG,",molecular_subtype ) ~ 
      gsub(", LGG,",",",molecular_subtype ),
    # if NA keep as "To be classified"
    molecular_subtype == "NA" ~ 
