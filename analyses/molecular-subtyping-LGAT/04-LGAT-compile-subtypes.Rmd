...
all_subtype <- bind_rows(all_rna_subtype,
  ...
  # there are some instances where LGAT DNA/RNA have multiple subtypes
  # so merging them here with summarise
  summarise(molecular_subtype= toString(molecular_subtype)) %>%
  # remove redudant ",LGG," from instances 
  # like molecular_subtype ==LGG, BRAF V600E, LGG, CDKN2A/B
  mutate(molecular_subtype = gsub(", LGG,",",",molecular_subtype )) %>%
  unique() %>%
