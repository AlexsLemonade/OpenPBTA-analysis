...
2. Any sample with "Neuroblastoma" in the `pathology_diagnosis` column, where `primary_site` does not contain "Other locations NOS", `pathology_free_text_diagnosis` does not contain "peripheral", and `pathology_diagnosis` or `pathology_free_text_diagnosis` does not contain "metastatic".
...
Note that the logic here also captures "Ganglioneuroblastoma" samples which will classify later in the module as `molecular_subtype == CNS Embryonal, NOS` per [this comment](https://github.com/AlexsLemonade/OpenPBTA-analysis/pull/788#issuecomment-700717514).
...
No, it looks like we need to account for "metastatic" in both the `pathology_diagnosis` and the `pathology_free_text_diagnosis` columns as we still capture "metastatic neuroblastoma" samples when we filter on `pathology_diagnosis` only.
...
Remember that the logic here also captures "Ganglioneuroblastoma" samples on the basis of the `pathology_diagnosis` column.
