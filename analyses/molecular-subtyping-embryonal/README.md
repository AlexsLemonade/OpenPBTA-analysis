...
4. Any sample with "Neuroblastoma" in the `pathology_diagnosis` column, where `primary_site` does not contain "Other locations NOS", `pathology_free_text_diagnosis` does not contain "peripheral", and `pathology_diagnosis` or `pathology_free_text_diagnosis` does not contain "metastatic" [per the same comment as above](https://github.com/AlexsLemonade/OpenPBTA-analysis/issues/752#issuecomment-697000066) and [this comment](https://github.com/AlexsLemonade/OpenPBTA-analysis/pull/788#discussion_r499948141).
Note that there are "Ganglioneuroblastoma" samples that are included with this logic, but given the `molecular_subtype` label `CNS Embryonal, NOS` as we are unable to subtype these samples to the `molecular_subtype` level due to unclear molecular lesions [per this comment](https://github.com/AlexsLemonade/OpenPBTA-analysis/pull/788#issuecomment-700717514).
