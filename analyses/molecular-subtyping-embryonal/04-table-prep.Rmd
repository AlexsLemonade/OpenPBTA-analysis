...
> * CNS Neuroblastoma tumor where primary site does not contain "Other locations NOS"
> * Pathology free text diagnosis does not contain "peripheral"
> * Pathology diagnosis or pathology free text diagnosis does not contain "metastic"

```{r}
all_data_df %>%
  filter(
    FOXR2_fusions != "None" | FOXR2 > 3 |
      pathology_diagnosis == "Neuroblastoma", # Note that we do not include "Ganglioneuroblastoma" samples here as they will be classified as `CNS Embryonal, NOS` per this comment: https://github.com/AlexsLemonade/OpenPBTA-analysis/pull/788#issuecomment-700717514
...
Although the sample `PT_MRTHX2AQ` fit the criteria above for labeling `CNS NB-FOXR2` tumors, it does not actually fit into the group based on molecular alterations per [this GitHub comment](https://github.com/AlexsLemonade/OpenPBTA-analysis/pull/788#pullrequestreview-502513095).
Therefore, we will this sample without a subtype label.
...
nb_foxr2_df <- all_data_df %>%
  ...
  mutate(
    molecular_subtype = case_when(
      Kids_First_Participant_ID == "PT_MRTHX2AQ" ~ "NA",
      TRUE ~ "CNS NB-FOXR2"
    )
  )
```
...
> * Tumors previously called Ganglioneuroblastoma.
