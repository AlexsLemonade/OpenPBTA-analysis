...
mn1_fusion_biospecimens <- fusion_summary_df %>%
  select(Kids_First_Biospecimen_ID, starts_with("MN1"))
```
...
embryonal_df <- embryonal_df %>%
  ...
  filter(str_detect(str_to_lower(pathology_diagnosis), 
           ...
           # "neuroblastoma" will also need to be detected in the `pathology_diagnosis` column, but we need to ensure that the `primary_site` != "Other Locations NOS" and the `pathology_free_text_diagnosis` column does note contain "peripheral"
...
Is there anything in the `pathology_free_text_diagnosis` column we should be concerned about removing?
...
No, nothing was changed to MB or ATRT.
