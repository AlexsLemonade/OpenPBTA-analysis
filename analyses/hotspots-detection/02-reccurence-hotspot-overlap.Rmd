...
# group by Kids_First_Participant_ID to check 
# for multiple dna samples 
clinical %>% 
  dplyr::filter(experimental_strategy != "RNA-Seq",
                sample_type=="Tumor") %>%
  dplyr::filter(composition != "Derived Cell Line")%>%
  group_by(Kids_First_Participant_ID) %>%
  # collapse the tumor_descriptors per Kids_First_Participant_ID
  summarise( 
...
```
We have instances of multiple samples per Kids_First_Participant_ID so will use the independent selected sample list from `independent-samples` module.
...
## Get combined and filtered maf calls from strelka,mutect, vardict, lancet
```{r}
combined_maf_subsets<- readRDS( file.path("results","combined_maf_subsets.RDS"))
...
summarise_sites <- combined_maf_subsets %>%
  # get gene annotation from gene reference
    left_join(gene_table,by="Hugo_Symbol") %>%
  # check if gnomad_AF is > 0.001 
  ...
  # summarise the following columns per genomic site 
  ...
  # annotate if amino acid position is available in MSKCC hotspot
	left_join(hotspot_database,by=c("Amino_Acid_Position","Hugo_Symbol")) %>%
  # annotate if gene is in Cosmic Census gene
  ...
  # filter only brain-goi
  dplyr::filter( grepl("brain-goi",type)) %>%
  # select independent samples only 
...
### Upset plots for the unique hits 
...
# in calls_recurrence
plotVaf(uniqhitsmaf,vafCol = "VAF",genes = uniqhitsGenes)
...
# Here the VAF column is unique to each Tumor_Sample_Barcode 
# which we didn't consider as a column on which to count recurrence.
# So the values here maybe difference compared to what we find
# in calls_recurrence
