...
Count up how many callers call SNP mutation. 

```{r}
# Add up how many callers identify each SNP mutation
caller_mat_snp <- combined_maf_subsets %>% 
...
# Add up how many callers identify each INDEL mutation
caller_mat_indel <- combined_maf_subsets %>% 
...
SNP calls
```{r}
UpSetR::upset(data = caller_mat_snp[,c("lancet","mutect","strelka","vardict")] ,
              order.by = "freq",main.bar.color = "gray")
```

INDEL calls
```{r}
UpSetR::upset(data = caller_mat_indel[,c("lancet","mutect","strelka","vardict")] ,
              order.by = "freq",main.bar.color = "gray")
```
...
uniqhitsGenes<-calls_recurrence %>% 
  left_join(combined_maf_subsets) %>% 
  dplyr::filter(is.na(hotspot_database) ) %>%
  pull(Hugo_Symbol) %>%
  unique()
