...
caller_mat <- combined_maf_subsets %>% 
...
## Upset graphs
```{r}
UpSetR::upset(data = caller_mat[,c("lancet","mutect","strelka","vardict")] ,
...
calls_recurrence <- summarise_sites %>%
    count(Chromosome,Start_Position,End_Position,Amino_Acid_Position,Hugo_Symbol,type,gnomad_AF_common,hotspot_database,Variant_Classification,sort = TRUE) %>%
  dplyr::filter(n>2)
