...
clin.rna <- subset.clin  %>%
  ...
  mutate(parent_aliquot_id = gsub('[.].*', '', parent_aliquot_id)) %>%
  dplyr::rename(Kids_First_Biospecimen_ID_RNA = Kids_First_Biospecimen_ID) %>%
  dplyr::select(Kids_First_Participant_ID, sample_id, parent_aliquot_id, tumor_descriptor, Kids_First_Biospecimen_ID_RNA)
...
clin.wgs <- subset.clin  %>%
  ...
  dplyr::select(Kids_First_Participant_ID, sample_id, parent_aliquot_id, tumor_descriptor, Kids_First_Biospecimen_ID_DNA)

# combine both
clin.merged <- clin.wgs %>%
  right_join(clin.rna, by = c('sample_id', 'Kids_First_Participant_ID','tumor_descriptor', 'parent_aliquot_id'))
```
</details>
