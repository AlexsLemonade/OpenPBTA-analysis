...
calc.consensus <- function(mm2s, medullo.classifier, clin){
  ...
  # add molecular subtype back reported from pathology report where there is no consensus
  consensus.mat.out = consensus.mat.out %>%
    mutate(molecular_subtype = ifelse(MM2S_best_fit == medulloPackage_best_fit, MM2S_best_fit, pathology_subtype))
...
* Consensus tabs: For cases in which both classifiers agree, molecular subtype reflects the consensus prediction. For cases in which there is no consensus prediction, but we have a pathology_subtype, the molecular subtype reflects the pathology subtype.
