...
# This was created in 00-mb-select-pathology-dex.Rmd
clinical_file <- file.path(data_dir, "pbta-histologies.tsv")

# set results directory
results_dir <- file.path(root_dir, "analyses", "molecular-subtyping-MB", "results")

# read in the terms file
...
# read input
subset.clin <- read.delim(subset_clinical_file) %>% 
...
calc.consensus <- function(mm2s, medullo.classifier, clin){
  ...
  consensus.mat <- consensus.mat.out %>%
    mutate(Kids_First_Biospecimen_ID = Kids_First_Biospecimen_ID_RNA) %>%
    dplyr::select(Kids_First_Biospecimen_ID, sample_id, pathology_subtype, other_molecular_findings, 
  ...
  # return as list
  output <- list(consensus.mat = consensus.mat, 
                 consensus.mat.out = consensus.mat.out, 
