...
# read expected and observed data
...
compute_accuracy <- function(expected_input, observed_input){
  ...
  # this is required to enable datatable filter
...
out <- compute_accuracy(expected_input = exp_class, 
                        observed_input = obs_class$`corrected_mat_medullo-classifier`)

# accuracy
medullo_classifier_corrected_acc <- out$accuracy
print(paste0("Accuracy: ", medullo_classifier_corrected_acc))

# output table
medullo_classifier_corrected <- out$merged_table
viewDataTable(medullo_classifier_corrected)
```
...
```{r, echo = TRUE, warning = FALSE, message = FALSE}
out <- compute_accuracy(expected_input = exp_class, 
                        observed_input = obs_class$`uncorrected_mat_medullo-classifier`)

# accuracy
medullo_classifier_uncorrected_acc <- out$accuracy
print(paste0("Accuracy: ", medullo_classifier_uncorrected_acc))

# output table
medullo_classifier_uncorrected <- out$merged_table
viewDataTable(medullo_classifier_uncorrected)
```

### Final output (batch-corrected)

```{r, echo = TRUE, warning = FALSE, message = FALSE}
out <- create_final_output(classifier_output = medullo_classifier_corrected,
                           clin = clin_merged)
...
classifier_output_tsv <- classifier_output_tsv %>%
  ...
  dplyr::mutate(molecular_subtype = if_else(!is.na(molecular_subtype),
                                            ...
                                            "MB, To be classified")) %>%
  write.table(file = file.path(results_dir, 'MB_batchcorrected_molecular_subtype.tsv'),
            row.names = F, quote = F, sep = "\t")
```

### Final output (uncorrected)

```{r, echo = TRUE, warning = FALSE, message = FALSE}
out <- create_final_output(classifier_output = medullo_classifier_uncorrected,
                           clin = clin_merged)
