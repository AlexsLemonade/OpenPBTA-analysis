...
This notebook consumes result file `gsva_scores_stranded.tsv` produced by this module's script `01-conduct-gsea-analysis` run with the `--apply_tumor_purity_threshold` flag.
...
From JL for next steps:
up in LGG compared to MB
up in LGG compared to EPN
up in Cranio compared to MB
up in cranio cpmpared to EPN

For this I need to write a function that compares individual cancer groups.
...
> Originally, 4/20 pathways were upregulated and 1/20 downregulated. After thresholding, 2/19 are upregulated.
> This is broadly consistent although both sets of comparisons are mostly NS.

```{r}
compare_original_threshold(tukey_original, tukey_thresholded, "HALLMARK_INTERFERON_ALPHA_RESPONSE", "Other lowgrade glioma")
```


#### Pilocytic astrocytoma

> Originally, 7/20 pathways were upregulated. After thresholding, 3/19 are upregulated.
> This is broadly consistent, but again, we have more NS that one might prefer for making strong conclusions about LGGs here.

```{r}
compare_original_threshold(tukey_original, tukey_thresholded, "HALLMARK_INTERFERON_ALPHA_RESPONSE", "Pilocytic astrocytoma")
```

#### Subependymal Giant Cell Astrocytoma

> Originally, 10/20 pathways were upregulated. After thresholding, 3/19 are upregulated.
> This is broadly consistent though with loss of power

```{r}
compare_original_threshold(tukey_original, tukey_thresholded, "HALLMARK_INTERFERON_ALPHA_RESPONSE", "Subependymal Giant Cell Astrocytoma")
```
...
```{r}
compare_original_threshold(tukey_original, tukey_thresholded, "HALLMARK_INTERFERON_ALPHA_RESPONSE", "Pleomorphic xanthoastrocytoma")
```
