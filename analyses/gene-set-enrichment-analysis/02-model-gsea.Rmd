...
+ For each pathway, are GSVA scores significant different across `short_histology`? If so, which histologies are significantly different?

+ For each pathway, are GSVA scores significant different across `disease_type_new`? If so, which types are significantly different?
...
In other words, we perform an ANOVA (and associated postdoc Tukey test) for each hallmark as, `hallmark ~ grouping`. Users can specify the grouping variable.
