...
+ For each pathway, are GSVA scores significantly different across `integrated_diagnosis`? If so, which types are significantly different?
...
file_anova_stranded_integrated_diagnosis <- file.path(results_dir, "gsva_anova_stranded_integrated_diagnosis.tsv")
file_anova_polya_integrated_diagnosis    <- file.path(results_dir, "gsva_anova_polya_integrated_diagnosis.tsv")
...
file_tukey_stranded_integrated_diagnosis <- file.path(results_dir, "gsva_tukey_stranded_integrated_diagnosis.tsv")
file_tukey_polya_integrated_diagnosis    <- file.path(results_dir, "gsva_tukey_polya_integrated_diagnosis.tsv")
...
Here we perform a series of ANOVAs, for polyA and stranded libraries separately, to determine whether mean GSVA scores for a given grouping are significantly different across hallmarks (pathways). The given groupings examined here are `short_histology` and `integrated_diagnosis`. 
...
integrated_diagnosis_stranded_model_results <- gsva_anova_tukey(metadata_with_gsva, integrated_diagnosis, "stranded", SIGNIFICANCE_THRESHOLD) 

head(short_histology_stranded_model_results)
head(integrated_diagnosis_stranded_model_results)

## Save stranded library results
write_tsv(short_histology_stranded_model_results[["anova"]],  file_anova_stranded_short_histology)
write_tsv(integrated_diagnosis_stranded_model_results[["anova"]], file_anova_stranded_integrated_diagnosis)
write_tsv(short_histology_stranded_model_results[["tukey"]],  file_tukey_stranded_short_histology)
write_tsv(integrated_diagnosis_stranded_model_results[["tukey"]], file_tukey_stranded_integrated_diagnosis)
...
if (!(running_in_ci)){
  ...
  integrated_diagnosis_polya_model_results <- gsva_anova_tukey(metadata_with_gsva, integrated_diagnosis, "polya", SIGNIFICANCE_THRESHOLD)
  head(short_histology_polya_model_results)
  head(integrated_diagnosis_polya_model_results)

  ### Save polya library results
  write_tsv(short_histology_polya_model_results[["anova"]],     file_anova_polya_short_histology)
  write_tsv(integrated_diagnosis_polya_model_results[["anova"]],    file_anova_polya_integrated_diagnosis)
  write_tsv(short_histology_polya_model_results[["tukey"]],     file_tukey_polya_short_histology)
  write_tsv(integrated_diagnosis_polya_model_results[["tukey"]],    file_tukey_polya_integrated_diagnosis)
...
**How many `integrated_diagnosis` have significant ANOVAs across hallmark pathways, for each library?**
```{r}
integrated_diagnosis_stranded_model_results[["anova"]] %>% count(significant_anova)
if (!(running_in_ci))
{
  integrated_diagnosis_polya_model_results[["anova"]] %>% count(significant_anova)
}
```
