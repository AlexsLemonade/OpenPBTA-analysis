...
* `10-tp53-tumor-purity-threshold.Rmd` specifically reassess results generated from the filtered tumor purity stranded data and compares them to original results with the full stranded data.
...
The following files are produced when this alternative pipeline is run.
...
- `stranded_tumor-purity-threshold_TP53_roc.png` 
  - Note that this is created by `06-evaluate-classifier.py`, not `07-plot-roc.R` as in the primary pipeline.
  This is because in the primary pipeline, `07-plot-roc.R` will overwrite the `matplotlib` plots produced by `06-evaluate-classifier.py`, but `07-plot-roc.R` is not run here so it is not overwritten.
