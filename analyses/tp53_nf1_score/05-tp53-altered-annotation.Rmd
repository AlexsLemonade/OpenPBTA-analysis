...
consensus_tp53_consensus_snv_indel <- data.table::fread(
...
consensus_tp53_hotspots_snv_indel <- data.table::fread(
  file.path(root_dir, "analyses" , "hotspots-detection" , "results" , "pbta-snv-scavenged-hotspots.maf.tsv.gz"),
                                   ...
                                   data.table = FALSE) %>%
  filter(Hugo_Symbol == "TP53") %>%
  select(colnames(consensus_tp53_consensus_snv_indel))


consensus_tp53_snv_indel <- consensus_tp53_consensus_snv_indel %>%
  bind_rows(consensus_tp53_hotspots_snv_indel) %>%
