...
library("ggthemes")
library("tidyverse")
score_stranded_df<-read_tsv("results/scores_df_stranded.tsv") %>% dplyr::select("TP53_SCORE","tp53_status","sample_id","Kids_First_Biospecimen_ID","Kids_First_Participant_ID") %>% unique()
...
score_polya_df<-read_tsv("results/scores_df_polya.tsv") %>% dplyr::select("TP53_SCORE","tp53_status","sample_id","Kids_First_Biospecimen_ID","Kids_First_Participant_ID") %>% unique()
...
tp53_status<-read_tsv("results/TP53_NF1_snv_alteration.tsv") %>%
  # select only tp53
...
tp53_mutant_status_score<-score_stranded_df %>%
  ...
  dplyr::left_join(tp53_status,by=c("sample_id"))
