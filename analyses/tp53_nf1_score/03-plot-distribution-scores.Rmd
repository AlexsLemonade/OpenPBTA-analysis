...
score_stranded_df <- read_tsv(file.path(results_dir, "score_status_stranded.tsv")) %>%
  dplyr::select(
    "TP53_SCORE",
    "tp53_status",
    "sample_id",
...
score_stranded_long <- reshape2::melt(score_stranded_df,
  id = c(
    "tp53_status",
    "sample_id",
    "Kids_First_Biospecimen_ID",
  ...
  dplyr::mutate(tp53_status = factor(tp53_status))

score_polya_df <- read_tsv(file.path(results_dir, "score_status_polya.tsv")) %>%
  dplyr::select(
    "TP53_SCORE",
    "tp53_status",
    "sample_id",
...
score_polya_long <- reshape2::melt(score_polya_df,
  id = c(
    "tp53_status",
    "sample_id",
    "Kids_First_Biospecimen_ID",
...
tp53_status <- read_tsv(file.path(results_dir, "TP53_NF1_snv_alteration.tsv")) %>%
  # select only tp53
...
```{r}
ggplot(score_stranded_long, aes(x = tp53_status, y = value)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(alpha = 0.5, width = 0.2) +
  stat_compare_means() +
  ggtitle("stranded") +
  theme_bw()
ggplot(score_polya_long, aes(x = tp53_status, y = value)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(alpha = 0.5, width = 0.2) +
  stat_compare_means() +
  ggtitle("polya") +
...
ggplot(tp53_mutant_status_score, aes(x = Variant_Classification, y = TP53_SCORE)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(alpha = 0.5, width = 0.2) +
  stat_compare_means() +
  theme_bw() +
...
score_stranded_df_non_mutant_ordered <- score_stranded_df %>%
  ...
  dplyr::filter(
    !is.na(Kids_First_Biospecimen_ID),
    # select for TP53 non mutant samples
    tp53_status == 0
  ) %>%
  head(250) %>%
  dplyr::left_join(histology,
    by = c(
      "sample_id",
      "Kids_First_Biospecimen_ID",
      "Kids_First_Participant_ID"
...
ggplot(score_stranded_df_non_mutant_ordered) +
  geom_bar(aes(x = broad_histology)) +
  coord_flip() +
  ylab("count") +
  xlab("broad_histology") +
  theme_bw() +
  ggtitle("Distribution of broad histology of TP53 non-mutant samples with high scores")
```
...
```{r}
# snv calls
snv_df <- data.table::fread(file.path(
  data_dir,
...
snv_df_goi <- snv_df %>%
  dplyr::filter(Hugo_Symbol != "TP53") %>%
  dplyr::left_join(histology[, c("Kids_First_Biospecimen_ID", "sample_id")],
    by = c("Tumor_Sample_Barcode" = "Kids_First_Biospecimen_ID")
  )

score_stranded_df_non_mutant_ordered_snv <- score_stranded_df_non_mutant_ordered %>%
  dplyr::left_join(snv_df_goi, by = c("sample_id")) %>%
  dplyr::filter(Variant_Classification %in% c(
    "Missense_Mutation",
    "Splice_Site",
    "Nonsense_Mutation",
    "Frame_Shift_Del",
...
# barplot for snvs
ggplot(score_stranded_df_non_mutant_ordered_snv) + 
  geom_col(aes(x = Hugo_Symbol, y = n)) + 
  coord_flip() + 
  ylab("count") + 
  xlab("broad_histology") + 
  theme_bw() + 
  ggtitle("Distribution of mutated genes in TP53 non-mutant samples with high scores")
```
