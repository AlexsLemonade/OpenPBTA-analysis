...
# 
# stranded_tp53 <- as.data.frame(t(stranded)[,"TP53"]) %>%
#   rownames_to_column() %>%
#   left_join(tp53_alterations_score_wide,by=c("rowname"="Kids_First_Biospecimen_ID_RNA")) %>%
#   rename(TP53='t(stranded)[, "TP53"]')
# 
# quantile_tp53 <- quantile(stranded_tp53[,"TP53"],probs=c(.25, .75), na.rm = FALSE)
#   
# rv_stranded <- matrixStats::rowVars(as.matrix(stranded))
# select <- order(rv_stranded, decreasing = TRUE)[1:5000]

# pcaobj_5000 <- stats::prcomp(t(stranded[select,]),
#                              rank. =5)
# # run pca
# summary(pcaobj_5000) # display top 5 PCs
# 
# expr_plot_anno <- pcaobj_5000$x %>%
#   as.data.frame() %>%
#   rownames_to_column() %>%
#   left_join(unique(tp53_alterations_score_wide[,c("Kids_First_Biospecimen_ID_RNA","tp53_altered","tp53_score")])
#             ,by=c("rowname"="Kids_First_Biospecimen_ID_RNA")) %>%
#   unique()%>%
#   as.data.frame() 
# 
# autoplot(pcaobj_5000,data = expr_plot_anno,colour = 'tp53_altered')
