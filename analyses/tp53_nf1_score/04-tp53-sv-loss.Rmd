...
# exon regions
exon <- exonicParts(txdb, linked.to.single.gene.only=FALSE)
# intron regions
intron <- intronicParts(txdb, linked.to.single.gene.only=FALSE)
...
```{r}
putative_onco_tp53_fusion_gr <- putative_onco_fusion %>%
...
bk_overlaps_exon
```
The breakpoint in BS_NJ4WPQVK overlaps the 1st exon of TP53

### Intron overlaps
To-do find the canonical intron start stop to check for overlap of intron 1
```{r}
...
bk_overlaps_intron
```
No breakpoints overlap TP53 introns
...
## Save fusion calls with TP53 loss
```{r}
