...
extraction_status <- read_tsv(file.path("results","pbta_extraction_status.tsv")) %>%
  select(-parental_aliquot_id)
  
```

## Add aliqout extraction status to base histology file
```{r}
base_histology <- base_histology %>%
  left_join(extraction_status, by = c("sample_id", "aliquot_id"))

```

## Add nucleic acid extraction details
