...
To make it easy for people to examine the mapping between these two fields, we'll write out a table to `results/`.
...
# additional modifications are made. We use the `cancer_group_pre_mod` column
# because including the subtypes would muddy the picture in the README.
mapping_table %>%
  dplyr::filter(additional_modification == "Yes") %>%
  dplyr::select(stripped_subtype = cancer_group_pre_mod,
                cancer_group) %>%
  dplyr::distinct() %>%
