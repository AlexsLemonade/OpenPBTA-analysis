...
## For v22 update only - create new base histologies from v21 file in order to update only subtyping and no other data
v21 <- read_tsv(file.path(data_dir,"pbta-histologies.tsv"), guess_max = 10000) 
v21 %>%
  select(-harmonized_diagnosis, -cancer_group) %>%
  write_tsv(file.path(data_dir,"pbta-histologies-base.tsv"))

## Usual start of the script
...
### Differences between V21 and V22 cancer group
```{r}
v21 %>%
...
### Check for differences in molecular subtypes
```{r}
base_histology_subset <- base_histology %>%
  select(Kids_First_Biospecimen_ID, molecular_subtype)

histology_subset <- histology %>%
  select(Kids_First_Biospecimen_ID, molecular_subtype)
dplyr::setdiff(histology_subset,base_histology_subset)
