...
vaf_df %>%
  reshape2::melt() %>% 
  dplyr::mutate(variable = gsub("^vaf.", "", variable)) %>% 
  dplyr::mutate(data.group = paste(dataset, ":", variable, "VAF")) %>%
  dplyr::filter(!is.na(value)) %>%
# Plot it
ggplot2::ggplot(ggplot2::aes(data.group, value)) + 
