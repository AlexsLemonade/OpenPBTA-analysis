...
strelka2_vaf <- strelka2@data %>%
  dplyr::select(Hugo_Symbol, Allele, Tumor_Sample_Barcode, Start_Position, 
                t_alt_count, t_ref_count, Reference_Allele) %>%
  ...
  dplyr::select(mutation, vaf, change)
...
```{r}
# Mutations common to both datasets:
vaf_df <- strelka2_vaf %>%
...
## Determine genes that are discordant.
...
Get a data.frame of the genes that differ most. 

```{r}
sig_diff_genes <- compare$results %>%
...
## What types of variants are these?

```{r}
vaf_df %>%
  dplyr::select(change.strelka2, change.mutect2, dataset) %>%
  tidyr::gather(key = "data", value = "change" , change.strelka2, change.mutect2) %>%
  dplyr::mutate('data' = gsub("^change.", "", data)) %>%
  dplyr::group_by(dataset, data, change) %>% 
  dplyr::summarise(count = n()) %>%
  dplyr::filter(count > 100, !grepl("^->", change), !is.na(change)) %>%
  ggplot2::ggplot(ggplot2::aes(x = change, y = count, fill = dataset)) + 
