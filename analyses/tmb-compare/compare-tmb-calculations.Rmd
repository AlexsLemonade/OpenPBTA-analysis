...
Calculate percent differences in relation to maftools-defined filtered TMB. 

```{r}
all_tmb <- all_tmb %>% 
  dplyr::mutate(perc_diff_focr = (tmb_maftools - tmb_focr) / tmb_maftools, 
                perc_diff_d3b = (tmb_maftools - tmb_d3b) / tmb_maftools)
```

Let's plot the distribution of percent differences from both d3b and focr-defined TMBs. 

```{r}
all_tmb %>% 
  dplyr::select(dplyr::starts_with("perc_diff")) %>% 
  tidyr::gather("tmb_calculation_set", "perc_diff") %>%
  dplyr::mutate(tmb_calculation_set = gsub("perc_diff_", "", tmb_calculation_set)) %>%
  ggplot2::ggplot(ggplot2::aes(x = perc_diff, color = tmb_calculation_set)) + 
  ggplot2::geom_density() + 
  ggplot2::theme_classic() + 
  ggplot2::ggtitle("Percent difference in TMB in comparison to CCDL maftools-defined TMB")
```
...
```{r}
all_tmb %>% 
  dplyr::filter(abs(perc_diff_focr) > .05) %>% 
  dplyr::arrange(desc(perc_diff_focr))
```

TMB is very different between CCDL and D3b calculations percent wise. 
...
Let' plot these counts correlations, but turn them into log1p values for easier visualization.
