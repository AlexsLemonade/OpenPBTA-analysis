...
FusionInNhist<-putative_driver_annotated_fusions %>% dplyr::select(FusionName,group) %>% unique() %>% group_by(FusionName) %>% summarise(count=n())
FusionInNhist<-FusionInNhist[FusionInNhist$count>countHistology,]
FusionInNhist<-FusionInNhist[order(FusionInNhist$count,decreasing = TRUE),]
FusionInNhist

putative_driver_annotated_fusions<-putative_driver_annotated_fusions %>% filter(!FusionName %in% FusionInNhist$FusionName)
