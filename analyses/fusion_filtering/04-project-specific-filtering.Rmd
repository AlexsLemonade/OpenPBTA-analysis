...
# remove read-throughs from all callers
readthrough<-read_tsv("scratch/readthroughs.tsv")
...
fusion_caller.summary <- fusion_calls %>%
  ...
  dplyr::select(-JunctionReadCount,-SpanningFragCount) %>%
...
fusion_read.summary <- fusion_calls %>%
  ...
  group_by(FusionName, Sample ) %>%
  unique() %>%
  dplyr::mutate(JunctionReadCount=sum(JunctionReadCount),SpanningFragCount=sum(SpanningFragCount))


fusion_calls<-full_join(fusion_caller.summary,fusion_read.summary,by=(c("Sample","FusionName","Fusion_Type","Gene1A","Gene2A","Gene1B","Gene2B","Gene1A_anno","Gene1B_anno", "Gene2A_anno", "Gene2B_anno", "Fusion_anno")))
...
fusion_calls$Distance<-unlist(lapply(fusion_calls$annots.x,function(x) ex_between(x, "LOCAL_REARRANGEMENT:-:[", "]")[[1]]))
fusion_calls<-fusion_calls %>% dplyr::filter(Distance>50000|is.na(Distance))
...
QCGeneFiltered_recFusionUniq<-QCGeneFiltered_recFusion %>%
  ...
  dplyr::filter(!FusionName %in% readthrough$readThroughs)
