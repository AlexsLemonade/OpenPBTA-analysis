...
def read_parameters():
     p = argparse.ArgumentParser(description=("The 01-snv-frequencies.py scripts creates copy number variation (CNV) cancer type and"      
                                             " study gene-level alterations frequencies table for the OPenPedCan analyses modules."),
                                formatter_class=argparse.RawTextHelpFormatter)
     p.add_argument('HISTOLOGY_FILE', type=str, default=None,
                    help="OPenPedCan histology file (histologies.tsv)\n\n")
     p.add_argument('CNV_FILE', type=str, default=None,
                    help="OPenPedCan CNV consensus file (consensus_seg_annotated_cn_autosomes.tsv or consensus_seg_annotated_cn_x_and_y.tsv)\n\n")
     p.add_argument('PRIMARY_TUMORS', type=str, default=None,
                    help="OPenPedCan independent primary tumor samples file (independent-specimens.wgs.primary.tsv)\n\n")
     p.add_argument('RELAPSE_TUMORS', type=str, default=None,
                    help="OPenPedCan independent relapse tumor samples file (independent-specimens.wgs.relapse.tsv)\n\n")
     p.add_argument('EFO_MONDO', type=str, default=None,
                    help="OPenPedCan disease to EFO/MONDO mapping file (efo-mondo-map.tsv)\n\n")
     p.add_argument('ENSG_RMTL', type=str, default=None,
                    help="OPenPedCan Ensembl to RMTL mapping file (ensg-hugo-rmtl-v*-mapping.tsv)\n\n")
     p.add_argument('-v', '--version', action='version',
                   version=f"classify_reads.py version {__version__} ({__date__})",
                   help="Print the current 01-snv-frequencies.py version and exit\n\n")
     return p.parse_args()
...
def merge_histology_and_cnv_data(histology_file, cnv_consensus_file):
	...
	histology_df = histology_df[["Kids_First_Biospecimen_ID","Kids_First_Participant_ID", 
	                                    "cohort", "cancer_group", "sample_type"]]

	# load CNV consensus file
	cnv_df = pd.read_csv(cnv_consensus_file, sep="\t")
	#cnv_df.head()

	# merge subset of histology dataframe to CNV dataframe keeping only sample
	# present in the CNV table (left outer join)
	merged_df = pd.merge(cnv_df, histology_df, how="left", left_on="biospecimen_id", 
	...
	if  not np.array_equal(np.array(["Tumor"]), merged_df.sample_type.unique()):
	    raise Exception(f"Merged hsitology-CNV dataframe contains non tumor samples")
	...
	all_tumors_df = merged_df[["gene_symbol", "ensembl", "status", "copy_number", "ploidy", "cohort",
	                  "cancer_group", "Kids_First_Biospecimen_ID", "Kids_First_Participant_ID"]].reset_index()
...
def get_cancer_groups_and_cohorts(all_tumors_df):
	...
	cancer_group_cohort_df.drop_duplicates(inplace=True, ignore_index=True)
...
def compute_variant_frequencies(all_tumors_df, primary_tumors_file, relapase_tumors_file, cancer_group_cohort_df):
	...
	primary_tumors_samples_list = list(pd.read_csv(primary_tumors_file, sep="\t")
	                              ["Kids_First_Biospecimen_ID"].unique())
	...
	relapse_tumors_samples_list = list(pd.read_csv(relapase_tumors_file, sep="\t")
	                              ["Kids_First_Biospecimen_ID"].unique())
	...
	for index, row in cancer_group_cohort_df.iterrows():
	    if row["num_samples"] > 5:
	        for df_name, tumor_df in tumor_dfs.items():
	            ...
	            if df.empty:
	                continue
	            num_pateints = df["Kids_First_Participant_ID"].nunique()
	            df = df.groupby(["ensembl", "status"]).apply(func)
	            df = df.rename_axis(["Gene_Ensembl_ID", "Variant_Type"]).reset_index()
	            df["num_pateints"] = num_pateints
	            for i, j in df.iterrows():
	                df.at[i, "Total_Alterations/Patients_in_Dataset"] = f'{j["total_alterations"]}/{num_pateints}'
	                df.at[i, "Frequency_in_Overall_Dataset"] = f'{(j["total_alterations"]/num_pateints)*100:.2f}%'
	                df.at[i, "Dataset"] = row["cohort"] 
	                df.at[i, "Disease"] = row["cancer_group"]         
	            df = df [["Gene_Symbol", "Gene_Ensembl_ID", "Variant_Type", "Dataset", "Disease", 
	                      "Total_Alterations/Patients_in_Dataset", "Frequency_in_Overall_Dataset"]]
	...
	primary_tumors_frequecy_df.rename(
	    columns={"Total_Alterations/Patients_in_Dataset": "Total_Primary_Tumors_Altered/Patients_in_Dataset",
	...
	relapse_tumors_frequecy_df.rename(
	    columns={"Total_Alterations/Patients_in_Dataset": "Total_Relapse_Tumors_Altered/Patients_in_Dataset",
	             "Frequency_in_Overall_Dataset": "Frequency_in_Relapse_Tumors"}, inplace=True)
	merging_list.append(relapse_tumors_frequecy_df)
	cnv_frequency_df = reduce(lambda x, y: pd.merge(x, y, how="outer", on=["Gene_Symbol", "Gene_Ensembl_ID", 
	                         "Variant_Type", "Dataset", "Disease"]), merging_list).fillna("")
	return(cnv_frequency_df)


def get_annotations(cnv_frequency_df, efo_mondo_mapping_file, rmtl_mapping):
	...
	for ensembl_gene in ensembl_gene_list:
	    ...
	    if (gene_name is None) or ("name" not in gene_name):
	        continue
	    cnv_frequency_df.loc[
	            cnv_frequency_df.Gene_Ensembl_ID == ensembl_gene, "Gene_Full_Name"] = gene_name["name"]
	...
	# populate CNV frequency dataframe with EFO and MONDO disease accessions              
	row_index = -1
	ensg_rmtl_df = pd.read_csv(rmtl_mapping, sep="\t")
...
def main():
	...
	cnv_frequency_df = get_annotations(cnv_frequency_df, args.EFO_MONDO, args.ENSG_RMTL)
	...
	results_tsv = f"{results_dir}/cnv-consensus-annotated-frequencies.tsv"
	cnv_frequency_df.to_csv(results_tsv, sep="\t", index=False, encoding="utf-8")
	# write results to TSV file	
	results_jsonl = f"{results_dir}/cnv-consensus-annotated-frequencies.jsonl"
