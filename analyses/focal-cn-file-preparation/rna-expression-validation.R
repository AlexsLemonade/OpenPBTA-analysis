...
merge_expression <- function(copy_number_df, expression_df, metadata){
  ...
  long_expression <- expression_df %>%
    ...
    dplyr::distinct() 
  long_expression$gene_id <- gsub(".*\\_", "", long_expression$gene_id)
...
#   ggplot2::geom_point(ggplot2::aes(col = label), size = 0.6) +
