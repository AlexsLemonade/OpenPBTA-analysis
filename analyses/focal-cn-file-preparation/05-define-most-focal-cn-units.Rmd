...
gene_cytoband_key <- combined_status_df$cytoband
names(gene_cytoband_key) <- combined_status_df$gene_symbol
```
...
if (!(running_in_ci)) {
  ...
  combined_status_count_df <- arm_status_count %>%
    ...
    rename(dominant_status.gene = dominant_status, 
           Kids_First_Biospecimen_ID = biospecimen_id) # Keep this consistent with the other datasets
} else {
  combined_status_count_df <- arm_status_count
}
...
combined_status_count_df %>% 
  ...
  gather(region_type, status, -Kids_First_Biospecimen_ID, -cytoband, gene_symbol) %>% 
  # Get rid of `dominant_status` prefix
  mutate(region = gsub("dominant_status.", "", region_type))
```

```{r}
# Write final long status table to file
write_tsv(final_long_status_df, 
