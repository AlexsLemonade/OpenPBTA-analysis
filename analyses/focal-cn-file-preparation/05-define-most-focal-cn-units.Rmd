...
status_majority_caller <- function(status_df,
  ...
  # Let's store the sample id values separately as well
  id_vector <- status_count %>%
    dplyr::pull(!!id_sym)
...
plot_dominant_status_calls <- function(status_count_df,
  ...
  status_count_df %>%
    ...
    select(-total, -region_variable, -biospecimen_id) %>%
    dplyr::ungroup() %>%
...
**Note:** There are gain status calls in this final data.frame.
This is not completely unexpected as there were many loss calls noticed in the data to begin with, but this may be something that is worth taking note of.
