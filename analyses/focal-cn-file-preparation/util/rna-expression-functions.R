...
plot_stacked_expression <- function (cn_expression_loss_df,
  ...
  #   loss_all_stacked_plot_name: name to save the first stacked plot displaying
  #                               expression levels of loss calls 
  #   neutral_all_stacked_plot_name: name to save the second stacked plot 
  ...
  # Plot and save
  cn_loss_expression_plot <- ggplot2::ggplot(cn_expression_loss_df,
    ...
    ggplot2::labs(title = toupper(gsub(".png", "", loss_all_stacked_plot_name)))
  
  ggplot2::ggsave(file.path(plots_dir, loss_all_stacked_plot_name),
                  cn_loss_expression_plot)
  
  cn_neutral_expression_plot <-
    ggplot2::ggplot(cn_expression_neutral_df,
                    ggplot2::aes(x = status,
                                 fill = expression_class)) +
    ggplot2::geom_bar(position = ggplot2::position_fill(reverse = TRUE)) +
    ggplot2::ylab("Proportion of called genes") +
    ggplot2::labs(title = toupper(gsub(
      ".png", "", neutral_all_stacked_plot_name
    )))
  
  ggplot2::ggsave(file.path(plots_dir, neutral_all_stacked_plot_name),
                  cn_neutral_expression_plot)
  
  cn_zero_expression_plot <- ggplot2::ggplot(cn_expression_zero_df,
                                             ggplot2::aes(x = status,
                                                          fill = expression_class)) +
    ggplot2::geom_bar(position = ggplot2::position_fill(reverse = TRUE)) +
    ggplot2::ylab("Proportion of called genes") +
    ggplot2::labs(title = toupper(gsub(".png", "", zero_all_stacked_plot_name)))
