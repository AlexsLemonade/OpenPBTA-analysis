...
no_estimate <- histologies_df %>% filter(sample_type == "Tumor") %>% filter(experimental_strategy == "WXS") %>%   filter(is.na(germline_sex_estimate)) %>% select(-germline_sex_estimate)
...
# Find out matching WGS - currently this has no entry for v9
no_estimate_participants_match_wgs <- histologies_df %>% filter(cohort_participant_id %in% no_estimate_participants) %>% filter(experimental_strategy == "WGS") %>% filter(sample_type == "Tumor") %>% select(cohort_participant_id, germline_sex_estimate)

# Merge data that was fixed with WGS germline sex estimate
no_estimate_fixed_through_wgs <- left_join(no_estimate, no_estimate_participants_match_wgs, by = "cohort_participant_id") %>% filter(!is.na(germline_sex_estimate))

# Identify data that was not fixed due to NA
no_estimate_still_na <- left_join(no_estimate, no_estimate_participants_match_wgs, by = "cohort_participant_id") %>% filter(is.na(germline_sex_estimate))
...
histologies_df_final <- histologies_df %>% 
  left_join(select(histologies_df_fixed, germline_sex_estimate, Kids_First_Biospecimen_ID), by = "Kids_First_Biospecimen_ID",
            suffix = c('.x', '.y'), suffixes_mandatory = c(FALSE, TRUE)) %>%
  mutate(germline_sex_estimate = case_when(Kids_First_Biospecimen_ID == Kids_First_Biospecimen_ID ~ germline_sex_estimate.y
                                           , TRUE ~ germline_sex_estimate.x)) %>% 
  select(-germline_sex_estimate.x, - germline_sex_estimate.y)
