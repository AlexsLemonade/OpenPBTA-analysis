...
```{r}
cnvkit_file <- file.path("..", "..", "data","v9", "cnv-cnvkit.seg.gz")
cnvkit_df <- readr::read_tsv(cnvkit_file)
```

```{r}
histologies_file <- file.path("..", "..", "data","v9", "histologies.tsv")
...
# Use cohort_participant_id for filtering samples that have WGS, when WXS was performed
no_estimate_participants_match_wgs <- histologies_df %>% 
  filter(cohort_participant_id %in% no_estimate_participants & 
