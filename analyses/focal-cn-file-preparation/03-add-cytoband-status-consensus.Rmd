...
```{r}
scratch_dir <- file.path("..", "..", "scratch", "cytoband_status")
```
...
final_df <- all_callable_cytoband_status_df %>%
  left_join(gains_cytoband_status_df,
            by = c("chr", "cytoband", "Kids_First_Biospecimen_ID")) %>%
  left_join(losses_cytoband_status_df,
            by = c("chr", "cytoband", "Kids_First_Biospecimen_ID"))
```
...
```{r}
# Replace NA's with 0
final_df$coverage_ratio_gains <- replace_na(final_df$coverage_ratio_gains, 0)
final_df$coverage_ratio_losses <- replace_na(final_df$coverage_ratio_losses, 0)
