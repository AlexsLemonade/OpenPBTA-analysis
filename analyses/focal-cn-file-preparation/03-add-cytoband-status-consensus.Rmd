...
final_df <- final_df %>%
  replace_na(list(
    coverage_ratio_gains = 0,
    coverage_ratio_losses = 0
  )) %>%
  group_by(chromosome_arm) %>%
  mutate(
    dominant_status = case_when(
      coverage_ratio_callable < 0.5 ~ "uncallable",
      coverage_ratio_gains / coverage_ratio_callable > 0.5 ~ "gain",
      coverage_ratio_losses / coverage_ratio_callable > 0.5 ~ "loss",
      (coverage_ratio_gains + coverage_ratio_losses) / coverage_ratio_callable > 0.5 ~ "unstable",
      TRUE ~ "neutral"
    )
  )
...
# Write to file
write_tsv(final_df, file.path("results", "consensus_seg_with_ucsc_cytoband.tsv.gz"))
```
...
final_gistic_df <- final_gistic_df %>%
  ...
  group_by(chromosome_arm) %>%
  # Keep only the first of the `dominant_status` values for a chromosome arm
  # when there are more than one -- the `domaninant_status` values are arranged,
  # therefore, "neutral" will be the second of two values when paired with
  # "loss" or "gain"
  mutate(
    dominant_status = case_when(
...
# Display only rows where gistic arm status and our arm status agree
final_gistic_df %>%
  filter(dominant_status == gistic_arm_status)
```
