...
1) "Lymphoma", "Melanocytic tumor", "Other tumor", "Metastatic tumors", "Non-CNS tumor" are combined into a `Other non-CNS group` in `display_group`. 

2) `Benign tumor` and `Non-tumor` biospecimens are combined into a `Benign` group. 

3) `Other astrocytic tumor` samples are combined into the existing `Low-grade astrocytic tumor`.
...
histology_table <- working_metadata %>% 
  dplyr::mutate(
    ...
    display_group = forcats::fct_collapse(display_group,
      ...
      "other non-cns tumor" = other_tumor,
      "benign" = begnign
    ),
    )
```
...
Now that the string search steps and manipulations are done, let's restore better-looking capitalization. 
