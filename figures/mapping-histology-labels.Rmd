...
subset_colors <- display_group_df$display_group
names(subset_colors) <- sample(color_palette, n_colors)
```

We want `Other tumor` and the `Benign` group to both always be gray. 

```{r}
names(subset_colors)[subset_colors == 'other tumor'] <- "#D3D3D3"
names(subset_colors)[subset_colors == 'benign'] <- "#808080"
```
...
```{r}
pie(rep(1, n_colors), 
    col = names(subset_colors), 
    labels = subset_colors)
```
