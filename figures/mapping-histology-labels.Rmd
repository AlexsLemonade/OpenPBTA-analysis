...
included_labels_df <- histologies_df %>% 
  ...
  mutate(included = case_when(
    n < 10 ~ FALSE,
    is.na(cancer_group) ~ FALSE,
    TRUE ~ TRUE
  ))
```

Let's take a look at only the `broad_histology` and `cancer_group` we'll create a palette for. 
...
So the unique values for `broad_histology` and `cancer_group` in `included_labels_df` are what we need to take into account for our palette.
