...
```{r}
if (nrow(display_group_df) > 17) {
  stop("There's more than 16 histology `display_group` may want to re-evaluate the high-level histology groupings")
}
```
...
We will always want the "normal", "benign", "other_tumor" groups to come last so we are gonna remove them from this ranking and then add back when we get to `histology_table`. 

```{r}
control_like_groups <- c("normal", "benign", "other tumor")

display_order_df <- display_group_df %>% 
  dplyr::filter(!display_group %in% control_like_groups) %>%
  dplyr::mutate(display_order = as.numeric(order(n, decreasing = TRUE))) %>% 
  dplyr::select(-n)
```
...
```{r}
max_rank <- max(display_order_df$display_order)

histology_table <- histology_table %>%
  # Join on the display orders
  dplyr::left_join(display_order_df, by = "display_group") %>% 
  # Add ranks for the control like groups
  dplyr::mutate(display_order = dplyr::case_when(
    display_group == "benign" ~ max_rank + 1, 
    display_group == "other tumor" ~ max_rank + 2, 
    display_group == "normal" ~ max_rank + 3, 
    TRUE ~ display_order
    ))
```
