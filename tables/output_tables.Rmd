...
# Gather all PT_IDs (poi = "patients of interest") for those with any hypermutant samples
poi <- tmb_hist %>%
...
# Gather all BS_IDs (soi = "samples of interest"), whether hypermutant or not, for pts with hypermutant samples
  soi_df <- tmb_hist %>%
    filter(Kids_First_Participant_ID %in% poi) %>%
    ...
    # keep only WGS samples; remove cell lines & metastases 
    filter(experimental_strategy == "WGS" & composition != "Derived Cell Line" & short_histology != "Metastases") %>%
    # add manual fields
    mutate(`Therapy post-biopsy` = case_when(
      ...
      Kids_First_Biospecimen_ID == "BS_P0QJ1QAH" ~ "Nivolumab",
      Kids_First_Biospecimen_ID == "BS_P3PF53V8" ~ "Temozolomide, Irinotecan, Radiation",
      Kids_First_Biospecimen_ID == "BS_02YBZSBY" ~ "Unknown"
    ),
    # add Lynch syndrome
    cancer_predispositions = case_when(
      ...
      Kids_First_Biospecimen_ID == "BS_P0QJ1QAH" ~ "Lynch Syndrome, pathogenic MSH2",
      Kids_First_Biospecimen_ID == "BS_02YBZSBY" ~ "Lynch Syndrome",
      TRUE ~ as.character(cancer_predispositions)),
      ) %>%
    # select relevant columns
      select(Kids_First_Participant_ID, Kids_First_Biospecimen_ID, sample_id, tumor_descriptor, `Therapy post-biopsy`, cancer_predispositions, tmb, molecular_subtype) %>%
