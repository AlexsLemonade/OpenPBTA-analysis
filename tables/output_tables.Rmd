...
input_cns_matches <- purrr::imap_dfr(jsonlite::fromJSON(CNS_match_json), 
                                     function(x, name) { tibble::tibble(CNS_region = name, primary_site = x) })

cns_regions_df <- input_cns_matches %>%
  group_by(CNS_region) %>%
  summarise(primary_site = str_c(primary_site, collapse = "|"))
